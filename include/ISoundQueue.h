...
enum E_SOUNDQUEUE_FLAG
{
    ...
    ESF_UNTIL_NEXT =
        1 << 3, // sound will end as soon as another available (added to queue)
    ESF_BREAK_LOOP = 1 << 4 // if the previous sound is looped, sound will start
                            // at end of its current loop.
};
...
public:
    ...
    // Add a sound to the queue. It will be played as soon as the given flag's
    // condition is met. sound - a unique sound created from ISoundSystem
    ...
    // I've forgotten what this does exactly. Something pretty darned awesome I
    // expect.
