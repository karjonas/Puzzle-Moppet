...
RenderTask::RenderTask(World *world)
{
    ...
    shaderManager =
        new ShaderManager(engine->GetIrrlichtDevice()->getVideoDriver());
...
IShader *RenderTask::CreateShader(const c8 *vertexShaderFileName,
                                  const c8 *pixelShaderFileName,
                                  video::E_MATERIAL_TYPE baseMaterial)
{
    return shaderManager->CreateShader(vertexShaderFileName,
                                       pixelShaderFileName, baseMaterial);
}
...
void RenderTask::ScreenFade(f32 fadeAmount, f32 time, bool fadeGUI)
{
    ...
    oldScreenFade = newScreenFade; // set to the end state of the last applied
                                   // fade, even if it hasn't finished.
...
void RenderTask::ScreenFadeFromCurrent(f32 fadeAmount, f32 time, bool fadeGUI)
{
    ...
    Less time is time = time * (newScreenFade-currentScreenFade) /
    (newScreenFade-oldScreenFade)
    */

    this->fadeGUI = fadeGUI;
    oldScreenFade =
        currentScreenFade; // this time we go from the current fade state
    newScreenFade = fadeAmount;
    fadeStartTime = GetVirtualTime();
    fadeFinishTime = fadeStartTime + time *
                                         (newScreenFade - currentScreenFade) /
                                         (newScreenFade - oldScreenFade);
...
void RenderTask::Update(f32 dt)
{
    ...
    // - Through graphics, set transform to interpolated (from last logic
    // transform to current transform)
    ...
    for (u32 i = 0; i < graphics.size(); i++)
    {
        ...
        graphics[i]->ReceiveRenderPosition(
            graphics[i]->GetInterpolatedAbsolutePosition(
                logicInterpolationAlpha));
        graphics[i]->ReceiveRenderRotation(
            graphics[i]->GetInterpolatedAbsoluteRotation(
                logicInterpolationAlpha));
    }

    // Multipass rendering.
    // Need to first count the maximum passes required (find which IGraphic has
    // greatest GetPassCount()). Then smgr->drawAll and IGraphic.Render for each
    // pass, calling SetCurrentPass on *ALL* IGraphics before each drawAll.

    // Count number of passes (through all IGraphics, and find the maximum pass
    // count) Default to at least one pass so that non Litha Engine managed
    // objects (other Irrlicht scene nodes) get rendered.
    u16 passCount = 1;
    ...
    {
        // If a post processing chain is set, we must first render to its input
        // texture, and then process it
        if (ppChain->GetInputTexture())
        {
            driver->setRenderTarget(ppChain->GetInputTexture(), true, true,
                                    backgroundCol);
        ...
        // Perhaps render screen. (optional when post processing; post
        // processing may completely replace it) Screen must be cleared and then
        // rendered to *after* Process(), as Process() may modify frame buffer.
    ...
    // This fixes an Irrlicht bug in 1.7+ where rendering something with a RTT
    // causes GUI rendering to be messed up. (GUI textures vertically flipped?)
    video::SMaterial mat;
    ...
    if (renderInvisible)
    {
        driver->draw2DRectangle(
            video::SColor(255, backgroundCol.getRed(), backgroundCol.getGreen(),
                          backgroundCol.getBlue()),
            core::recti(0, 0, driver->getCurrentRenderTargetSize().Width,
                        driver->getCurrentRenderTargetSize().Height));
    }
...
void RenderTask::Render(u16 passCount)
{
    ...
    for (u16 i = 0; i < passCount; i++)
    {
        ...
        // The most likely use for this is to modify the materials of any
        // wrapped irrlicht scene nodes. This is called for each pass of a
        // graphic irrespective of whether the graphic has the pass set. This is
        // so the graphic can set any scene nodes invisible if they are not to
        // be rendered this pass.
        ...
        // NOTE: in most cases graphics Render methods will not be overridden
        // and will do nothing, as graphics rendering (of Irrlicht scene nodes)
        // is usually handled by Irrlicht internally. (and rendered in
    ...
    // Really need an IVideoDriver method that defines a render target that 2D
    // images are drawn relative to. (usually it uses the current render target,
    // I would want to change it so it always uses the screen render target
    // dimensions) render target dimensions are gotten in setRenderStates2DMode.
...
void RenderTask::RenderFade()
{
    ...
    if (fadeTimePeriod > 0.0 && currentTime < fadeFinishTime)
    {
        ...
        {
            // so now current fade must be set to that fraction between old and
            // new fades.
            currentScreenFade =
                oldScreenFade + (newScreenFade - oldScreenFade) * alphaTime;
        }
    }
    else
    {
    ...
    // Draw faded rect
    driver->draw2DRectangle(
        video::SColor((u32)((1.0 - currentScreenFade) * 255), 0, 0, 0),
        core::recti(0, 0, driver->getCurrentRenderTargetSize().Width,
                    driver->getCurrentRenderTargetSize().Height));
}
