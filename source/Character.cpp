...
// Fixes needed:
// - PID controller to maintain orientation when on a slope? (created when keys
// not pressed)
// - user mass setting?
// - material? default material? user material? what about interactions?
//		- material might NOT want setting.... as not all character models rely
// on this (e.g. lollipop)
//		- and this is really an internal setting...
// - user damping amount setting?

Character::Character(IWorld *world, IPhysics *physics, IMesh *mesh,
                     s32 idAnimIdle, s32 idAnimWalk, f32 height)
{
    this->world = world;
    this->mesh = mesh;
    ...
    // Offset the mesh so that its bottom is level with the bottom of the
    // geometry.
    f32 geomHeight = geom->GetSize().Y;
    f32 meshSink =
        -mesh->GetBoundingBox().MinEdge.Y; // amount by which the mesh
                                           // encroaches under zero Y.
...
{
    return this->idAnimIdle == idAnimIdle && this->idAnimWalk == idAnimWalk;
}
...
void Character::Update(f32 dt)
{
    ...
    if (relativeMotion.getLengthSQ() != 0.0)
    {
        ...
        // (X and Z are switched because rotation about X axis causes movement
        // along Z, and vice versa)
        body->ApplyTorque(core::vector3df(actualMotion.Y, 0.0, actualMotion.X) *
                          moveSpeed);
        ...
        else
        {
            ...
            SetRotation(
                core::vector3df(0,
                                maths::interpolate_angle(currentRotY, faceAngle,
                                                         turnSmooth * dt),
                                0));
        }
    }
    ...
    mesh->SetPosition(body->GetPosition() +
                      core::vector3df(0, meshHeightOffset, 0));
}
