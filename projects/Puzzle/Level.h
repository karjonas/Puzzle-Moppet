...
struct MapObjectMove
{
    ...
    core::vector3di startMapCoord; // added this back in, since we need it for
                                   // OnLeaveLocation
    core::vector3df startMapPos;   // changed to float as player may not have
                                   // exact map position to interpolate from
...
struct Actor
{
    // Character is proxied as a slightly convoluted way to give a position
    // offset.
...
enum TUTORIAL_TRIGGER_TYPE
{
    ...
    TTT_Y,
    TTT_Z,
    TTT_SURROUND // when hits any of 8 squares surrounding this square (but
...
class Level : public IUpdatable, public IWantInput, public IWantEvents
{
    ...
    std::vector<core::vector3di> locationsLeftNew; // new location of each of
                                                   // those left

    // A default event called for each map location entered (as well as custom
    // events)
    ...
    // so we get footstep sounds when walking on sand (and there are no blocks
    // underneath)
...
public:
    void ForceFootsteps(bool alwaysEnabled) { alwaysFootsteps = alwaysEnabled; }
    ...
    core::vector3di GetActorCoord(Actor &c)
    {
        return GetCoord(c.entity->GetPosition());
    }
    ...
    bool IsActorTouchingCoord(Actor &actor, core::vector3di mapCoord,
                              f32 scale = 1.0);
    ...
    void ApplyDefaultShaders(
        IMesh *mesh, video::E_MATERIAL_TYPE baseMaterial = video::EMT_SOLID);
    void ApplyDefaultShadersIrr(
        scene::IMesh *mesh,
        video::E_MATERIAL_TYPE baseMaterial = video::EMT_SOLID);
    void ApplyLandShaders(
        IMesh *mesh, video::E_MATERIAL_TYPE baseMaterial = video::EMT_SOLID);
    ...
    IMesh *AddObject(core::vector3di mapCoord, const c8 *meshName, bool movable,
                     E_OBJECT_TYPE type, bool solid = true);
    ...
    // Called by Update when the player performs a push action towards a map
    // location Note that this only includes a direct push, and will NOT be
    // called if the player touches a corner of the map coordinate.
    ...
    // Called as soon as an object starts moving. (unlike above which waits til
    // the move finishes) Also unlike above, this is NOT called on game start.
    // Only called when an object has actually moved. coord is the destination.
    ...
    // MainState pointer is now optional (can be NULL), since we may create a
    // level without mainstate existing for preview in start screen.
    Level(MainState *mainState, core::stringc fileName,
          std::deque<UndoState> *undoHistory = nullptr);
    ~Level();
    ...
    void SetLogicEnabled(bool logicEnabled)
    {
    ...
    // Starts an animated move of an object from one location to another, over
    // time Moves are updated and finished when necessary in Update
    bool StartMapObjectMove(core::vector3di source, core::vector3di dest,
    ...
    // There are hacks, and then there are *hacks*. This is in the latter
    // category. Used by some events/moving objects to push the player out of
    // the way. Only works on player intersecting squares, the actual centre
    // location occupied by the player cannot be moved. Basically, this just
    // removes the player's claim to this location. Collisions and physics are
