...
void MotionSensor::Update(f32 dt)
{
    ...
    else
    {
        ...
        // We want at least one set of values Add()ed to averages before calling
        // Get().
        if (skipCounter >= averagingSkip)
        {
            ...
            // for translation, we may only consider distances along some of the
            // axes.
            if (translateAxesEnabled == (AXIS_X | AXIS_Y | AXIS_Z))
                ...
                translateDist =
                    core::vector2df(pos.X, pos.Y)
                        .getDistanceFrom(core::vector2df(lastPos.X, lastPos.Y));
            else if (translateAxesEnabled == (AXIS_X | AXIS_Z))
                translateDist =
                    core::vector2df(pos.X, pos.Z)
                        .getDistanceFrom(core::vector2df(lastPos.X, lastPos.Z));
            else if (translateAxesEnabled == (AXIS_Z | AXIS_Y))
                translateDist =
                    core::vector2df(pos.Z, pos.Y)
                        .getDistanceFrom(core::vector2df(lastPos.Z, lastPos.Y));
            else
                translateDist = 0.f;
            ...
            rotDeltaHistory.push_back(
                maths::scalar_rotation_difference(rot, lastRot));

            // erase oldest history element
            if (posDeltaHistory.size() >
                averagingCount + 1) // +1, want at least one element
            {
        ...
        if (posDeltaHistory.size() > averagingCount)
        {
            ...
            bool isTranslating =
                posDeltaAverage.Get() >= dt * minTranslateSpeed;
            ...
            if (updateCount == 1)
            {
                ...
                // We now have one previous frame, so can tell if we are in
                // motion! Perhaps send initial events.
            ...
            {
                // Now have at least two previous frames, so we can detect if a
                // change in motion has occurred.
