...
public:
    ...
    {
        shader->SetPixelConstant("rippleScroll",
                                 engine->GetRenderUpdater().GetVirtualTime() *
                                     0.02);

        // Are we looking in to the sun?
        core::vector3df camVec = maths::rotation_to_direction(
            engine->GetWorld()->GetCamera()->GetRotation());

        sunDirection.normalize();

        f32 dp = camVec.dotProduct(sunDirection);
        // f32 brightness = 1.f + 0.5 *
        // dp*dp*dp*dp*dp*dp*dp*dp*dp*dp*dp*dp*dp*dp*dp*dp;
        f32 brightness =
            1.f + 0.5 * dp * dp * dp * dp * dp * dp * dp * dp * dp * dp;

        if (brightness < 1.f || camVec.getDistanceFrom(sunDirection) > 1.f)
            brightness = 1.f;

        brightness = core::lerp(
            lastBrightness, brightness,
            core::clamp(engine->GetRenderUpdater().GetLastDeltaTime(), 0.f,
                        1.f));
...
// Mac passes a -psn (process serial number) argument when running from a .app
// directory, which messes this up. So we just ignore command line arguments on
// mac for now. Ideally though, we'd just strip that out and ignore it.
#ifndef __APPLE__
    if (argc == 3)
    {
        // Edit?
...

    {
        ...
        rect += core::vector2di(screenWidth / 2 - rect.getWidth() / 2,
                                screenHeight / 2 - rect.getHeight() / 2);
    ...
    {
        // bool flag indicates whether rendering to screen still occurs first
        // before post processing
        IPostProcessingChain *chain =
            renderSystem->CreatePostProcessingChain(true);
        ...
        // Bright filter
        shader =
            renderSystem->CreateShader("ScreenQuad.vert", "Brightfilter.frag");
        ...
        // Vertical blur
        // As this is the last effect it is rendered additively on to the
        // screen.
        shader = renderSystem->CreateShader("ScreenQuad.vert", "BlurV.frag",
    ...
    {
        IShader *skyShader =
            renderSystem->CreateShader("SkyBox.vert", "SkyBox.frag",
                                       video::EMT_SOLID);
    ...
    if (renderSystem->ShadersAreAvailable())
    {
        ...
        shader = renderSystem->CreateShader("Hemisphere.vert",
    "Hemisphere.frag", video::EMT_SOLID); shader->drop();

        shader = renderSystem->CreateShader("HemisphereBalloon.vert",
    "HemisphereBalloon.frag", video::EMT_SOLID); shader->drop();

        shader = renderSystem->CreateShader("HemisphereLand.vert",
    "Hemisphere.frag", video::EMT_SOLID); shader->drop();

        shader = renderSystem->CreateShader("Hemisphere.vert",
    "HemisphereWood.frag", video::EMT_SOLID); shader->drop();

        shader = renderSystem->CreateShader("PlainWithAlpha.vert",
    "PlainWithAlpha.frag", video::EMT_SOLID); shader->drop();
    }
    */
    ...
    // ...unless the game was started by command line, in which case we jump
    // straight in.
    ...
    // MainState may not exist if StartScreen was entered but no level was
    // played.
    if (mainState)
        delete mainState;
