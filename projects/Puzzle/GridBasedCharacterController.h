...
class AlphaOnAnimator : public IMotionAnimator
{
    ...
    {
        ASSERT(mesh);
        scene::IMeshManipulator *mm = GetEngine()
                                          ->GetIrrlichtDevice()
                                          ->getSceneManager()
                                          ->getMeshManipulator();
...
public:
    GridBasedCharacterController(IWorld *world, int idButtonWalkForwards,
                                 int idButtonWalkBackwards,
                                 int idButtonWalkLeft, int idButtonWalkRight,
                                 ITransformable *moveRelativeTo, Level *level)
    {
        this->world = world;
    ...
    void EndMove() { isMovingIntoLocation = false; }
    ...
    void Update(ICharacter *character) override
    {
        ...
        if (moveRelativeTo)
        {
            ...
            // Find if move direction is within a certain angle difference from
        ...
        if (playerInputMoved && !isMovingIntoLocation && inputProfile &&
            character->GetAttemptedMove().getLengthSQ() > 0.001f && atMoveAngle)
        {
            // Cache button states
            // Move is continued automatically until the centre of the location
            // is reached.
            ...
            // Find centre of location we are destined to enter, that is not the
            // current location.
            ...
            core::vector3df destPosCentred(destMapCoord.X, destMapCoord.Y,
                                           destMapCoord.Z);

            // Distance from player to dest pos
            f32 continueDist =
                character->GetPosition().getDistanceFrom(destPosCentred);

            // Scale factor of time it takes to move one unit.
            // This is chosen experimentally...
            // It's dependent on player speed, but player has acceleration etc
            // so this is only an approximation.
            const f32 scaleFactor = 0.3f;

            finishEnteringTime =
                level->GetVirtualTime() + continueDist * scaleFactor;
        ...
        if (moveRelativeTo)
        {
            core::vector3di playerMapPos =
                level->GetCoord(character->GetPosition());

            bool showGridArrow = !playerInputMoved && !isMovingIntoLocation &&
                                 (level->GetMap()->GetObject(playerMapPos) &&
                                  !level->GetMap()->IsTraversing(playerMapPos));
            ...
            if (showGridArrow)
            {
                ...
                core::vector3df playerPosCentred =
                    level->GetPosFromCoord(playerMapPos);

                core::vector3df arrowPos =
                    playerPosCentred + core::vector3df(0.f, -0.475, 0.f);
        ...
        {
            // Moving into location. Just entered a location, so continue moving
            // automatically for a small amount of time (so player only ever
            // stops at the centre of a location).

            // We set motion in the same manner as below, but using cached
            // button states.

            // Maybe set medial motion

            if (buttonDownCache.forwards && !buttonDownCache.backwards)
            {
                character->SetMedialMotion(1);
            }

            if (buttonDownCache.backwards && !buttonDownCache.forwards)
            {
                character->SetMedialMotion(-1);
            }
            ...
            if (buttonDownCache.left && !buttonDownCache.right)
            {
                character->SetLateralMotion(-1);
            }

            if (buttonDownCache.right && !buttonDownCache.left)
            {
                character->SetLateralMotion(1);
            }
