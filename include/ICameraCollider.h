...
public:
    ...
    // Add some geometry to be excluded from collisions. Can be called multiple
    // times. e.g. might exclude a character's geometry.
    virtual void ExcludeGeometry(
        const Set<ICollisionGeometry *> &excluding) = 0;
    ...
    // cameraRay - represents the camera. Starts at the camera target and ends
    // at the camera position. resultPos - ProcessCollision should store the
    // modified camera position here. Returns true if a collision occurred and
    // resultPos has been modified. resultPos should NOT be modified if no
    // collision occurs.
    virtual bool ProcessCollision(const core::line3df &cameraRay,
                                  core::vector3df &resultPos) = 0;
};
