...
struct MapObject
{
    ...
    // Changed from IObject to IEntity so player (ICharacter) can also be set
    // here.
    ...
    // remember the previous location this object was at, before the
    // Map.StartMoveObject was called. Teleportation by SetObject does not set
    // this.
    core::vector3di previousCoord;
    bool previousCoordSet;

    // hacked in to fix undo bug
    // (otherwise objects may slide after undo, if the stopping object has since
    // moved)
    bool is_sliding;
};
...
class Map
{
    struct MapLocation
    {
        ...
        MapObject object; // may or may not be used (see containsObject flag
                          // below)
...
public:
    ...
    // IMPORTANT NOTE: This method should only really be used by
    // Level::StartMapObjectMove, so that moves applied to the player's square
    // can be intercepted and given special case treatment.
    bool StartObjectMove(core::vector3di source, core::vector3di dest,
                         bool forceMove = false);

    // hacked in for undo system.
    // same as StartObjectMove but doesn't actually do the move.
    bool ObjectMoveWillComplete(core::vector3di source, core::vector3di dest,
                                bool forceMove = false);

    // Call this to finalise the move. An animation can be performed between
    // start and finish. (during which time the object is flagged as
    // "traversing" and cannot be affected in an way)
    ...
    void SetObject(core::vector3di coord, ITransformable *object, bool movable,
                   E_OBJECT_TYPE type);
    ...
    // see GetObjectType
    // gets the previously occupied coordinate of the object at the specified
    // coordinate will fail if the object has never moved an immovable object
    // will (probably) never have a previous coord set.
    ...
    void SetObjectPreviousCoord(core::vector3di coord,
                                core::vector3di prevCoord);
