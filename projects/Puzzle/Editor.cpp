...
Editor::Editor(Level *level)
{
    ...
    // Disable logic for the level. Don't want movable things changing position
    // while editing.
    ...
    engine->GetIrrlichtDevice()->getGUIEnvironment()->setUserEventReceiver(
        this);
...
void Editor::AddBrushIcon(u16 row, u16 column, const c8 *brushIconFileName,
                          int id)
{
    ...
    core::recti rect(x + (iconDimensions.Width + padding) * column,
                     y + (iconDimensions.Height + padding) * row,
                     x + (iconDimensions.Width + padding) * (column + 1),
                     y + (iconDimensions.Height + padding) * (row + 1));
...
void Editor::CreateBrushSelector()
{
    ...
    AddBrushIcon(0, 1, "icon_event_player_start.png",
                 EOT_COUNT + EET_PLAYER_START_EVENT);
...
void Editor::OnButtonDown(int id)
{
    ...
    if (id == 0 && !inToolMode && selectedBrush) // No adding or deletion when
                                                 // in tool mode.
    {
        // hack
        ...
        else
        {
            ...
            {
                level->CreateEvent(targetCoord,
                                   (E_EVENT_TYPE)(selectedBrush - EOT_COUNT));
    ...
    {
        // Load
        // THIS IS NOW NOT DONE AT ALL. The only place a level is loaded for
        // editing is on start, when passed via command line.
        // level->Load();
        // RefreshEventDisplay();
    }
}
...
{
    // Need to update target cube's position... Keeping it tied to a grid
    // coordinate.

    const f32 targetCubeDistance = 5.0;

    core::vector3df cameraVec =
        (camera->getTarget() - camera->getPosition()).normalize();

    core::vector3df worldPos =
        camera->getPosition() + cameraVec * targetCubeDistance;
