...
public:
    ...
    f32 GetVirtualTime() const { return virtualTime; }

    f32 GetLastDeltaTime() const { return lastDeltaTime; }

    // Used in a similar manner to IEngine.QueueEvent.
    // Key difference: This event's delay will be paused when this Updatable is
    // paused.
    ...
    // This should be used after the app has frozen for a while (i.e. from
    // loading media) in order to prevent a sudden jump forwards in time.
    ...
    virtual void Update(f32 dt)
    {
        ...
        for (u32 i = 0; i < timedEvents.size(); i++)
        {
            if (timedEvents[i]["___PostTime___"].To<f32>() <= virtualTime)
            {
                ...
                // So will be sent by Kernel right after all Updatables have
                // finished updating.
