...
// Put the given ASCII string in the specified file. Overwrites any existing
// contents. Returns false on failure or partial failure.
...
// Put the given vector of strings into the specified file, each as a new line,
// using the current OS's newline sequence. Overwrites any existing contents.
// Returns false on failure or partial failure.
bool put_lines(const io::path &fileName,
               const std::vector<core::stringc> &lines);
