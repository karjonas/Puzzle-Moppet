    ...
    for (u32 i = 0; i < elements.size(); i++)
    {
        ...
        if (i == 0 && enabledLeft)
        {
            SetTopLeft(elements[i],
                       screenWidth / 2 - screenCentreOffset, // snug against
                                                             // screen edge
                       yPos - rect.getHeight() / 2 // Vertically centred
            );
            elements[i]->setAlignment(gui::EGUIA_UPPERLEFT,
                                      gui::EGUIA_UPPERLEFT, gui::EGUIA_CENTER,
                                      gui::EGUIA_CENTER);
        }
        else if ((i == 1 && enabledRight) ||
                 (i == 0 && !enabledLeft && enabledRight))
        {
            SetTopLeft(elements[i],
                       screenWidth / 2 + screenCentreOffset -
                           rect.getWidth(),        // snug against screen edge
                       yPos - rect.getHeight() / 2 // Vertically centred
            );
            elements[i]->setAlignment(gui::EGUIA_LOWERRIGHT,
                                      gui::EGUIA_LOWERRIGHT, gui::EGUIA_CENTER,
                                      gui::EGUIA_CENTER);
        }
    }
}
