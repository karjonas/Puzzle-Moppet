...
public:
    ...
    // eventQueue - the event queue that contains the wait that this logic is
    // for waitStartTime - time that this wait started (in the event queue's
    // virtual time)
...
public:
    ...
    // If no waits are present, this event will be sent immediately, and then
    // removed from the queue. (using IEngine.QueueEvent with time delay 0 - so
    // on the next kernel tick) If waits are present, event will be sent when
    // they finish.
    virtual void AddEvent(const Event &event) = 0;

    // Add a time wait to the queue.
    // This causes a delay of the given waitTime before the next item is
    // processed.
    virtual void AddTimeWait(f32 waitTime) = 0;

    // Add a logic wait to the queue.
    // This delays processing of the next item in the queue until the
    // IWaitLogic.IsWaitOver method returns true. (IsWaitOver is queried every
    // update of the EventQueue)
    ...
    // scale any time delays remaining. useful to run the queue at a different
    // speed.
