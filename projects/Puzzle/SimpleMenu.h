...
public:
    // A unique Id must be given which will identify events generated by this
    // menu.
    SimpleMenu(s32 uniqueId);
    virtual ~SimpleMenu();

    // removeMenuOnClick unused. Would be useful in future if there is ever a
    // MenuManager class, or if menus are managed by Litha Engine. Menu could
    // then auto remove itself by calling a manager method.
    virtual void AddItem(core::stringw name, s32 uniqueItemId,
                         bool removeMenuOnClick = false);

    virtual void AddImageItem(io::path imageName, s32 uniqueItemId,
                              bool removeMenuOnClick = false);

    // Space between items. Might be horizontal or vertical, depending on the
    // menu type.
    virtual void SetItemGap(u32 gap);

    // Should be called after all items have been added; creates GUI elements
