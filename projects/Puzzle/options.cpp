...
VariantMap get_current_config()
{
    ...
    settings["postProcessingEnabled"] =
        creationSettings["postProcessingEnabled"];
...
void apply_permanent_config(VariantMap config)
{
    ...
    // Should we restart?
    // (is any settings changed that require a restart? Current state != new
    // config state ?)
    ...
    {
        NOTE << "An option was changed that requires the application to "
                "restart.";
