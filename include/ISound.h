...
public:
    ...
    // whether the sound loops. If this is set false while the sound is already
    // playing, it will finish at the end of the current play. This state is
    // remembered across calls to Play(..). (so can be called before a call to
    // Play) Default state is not looped.
    virtual void SetIsLooped(bool loop) = 0;

    // Returns true if the sound has played to the end or if the sound has had
    // Stop() called.
