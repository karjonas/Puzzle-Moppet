...
{
public:
    void Prepare(CollisionMaterialInteraction &interaction,
...
bool Level::IsActorTouchingCoord(Actor &actor, core::vector3di mapCoord,
                                 f32 scale)
{
    ...
    core::aabbox3df box(mapPos - core::vector3df(0.5, 0.5, 0.5),
                        mapPos + core::vector3df(0.5, 0.5, 0.5));
...
std::vector<core::vector3di> Level::GetActorIntersectingCoords(Actor &actor)
{
    ...
    // (as it needs to take control of them so e.g. no falling block may fall
    // into them) [x][x][x] [x][P][x] [x][x][x]

    core::vector3di actorMapPos = GetCoord(actor.entity->GetPosition());

    // Find surrounding "shell" of map coordinates
    // Then draw a line from centre of Actor towards the centre of each of these
    // coordinates (as if each is a block). Line is length: player radius.
    ...
    for (int x = -1; x <= 1; x++)
    {
        ...
        for (int z = -1; z <= 1; z++)
        {
            ...
            {
                core::vector3di mapCoord =
                    actorMapPos + core::vector3di(x, 0, z);
...
{
    engine->GetIrrlichtDevice()->getSceneManager()->addCubeSceneNode(
        1.0, nullptr, -1, GetPosFromCoord(mapCoord));
}
...
{
    LowLevelShaderRegisterMap rmap;
    // rmap.PushSingleRegister("UseDiffuseTexture", "diffuseTexture",
...
public:
    ...
    void ShaderOnSetConstants(IShader *shader) override
    {
        ...
        shader->SetVertexConstant("timeSeconds",
                                  engine->GetRenderUpdater().GetVirtualTime());
    }
};

void Level::ApplyDefaultShaders(IMesh *mesh,
                                video::E_MATERIAL_TYPE baseMaterial)
{
    ...
    for (u32 i = 0; i < mesh->GetMaterialCount(); i++)
    {
        ...
        {
            IShader *shader =
                renderSystem->CreateShader("Hemisphere.vert", "Hemisphere.frag",
                                           baseMaterial);
            ...
            // shader->SetPixelConstant("UseDiffuseTexture",
            // material.TextureLayer[0].Texture ? 1 : 0);
...
void Level::ApplyDefaultShadersIrr(scene::IMesh *mesh,
                                   video::E_MATERIAL_TYPE baseMaterial)
{
    // Set up mesh materials (with shaders)
...
void Level::ApplyLandShaders(IMesh *mesh, video::E_MATERIAL_TYPE baseMaterial)
{
    ...
    for (u32 i = 0; i < mesh->GetMaterialCount(); i++)
    {
        ...
        {
            IShader *shader =
                renderSystem->CreateShader("HemisphereLand.vert",
                                           "Hemisphere.frag", baseMaterial);
            shader->SetVertexRegisterMap(get_vertex_shader_register_map());
            shader->SetPixelRegisterMap(get_pixel_shader_register_map());
...
/*
// Exactly as land above but with EMT_TRANSPARENT_VERTEX_ALPHA and a different
frag shader.
// This shader uses colour values as alpha, since Blender will not let you paint
vertex alphas... void Level::ApplyCliffGrassShaders(IMesh *mesh)
{
    // Set up mesh materials (with shaders)
    for (u32 i = 0; i < mesh->GetMaterialCount(); i ++)
    {
        ...
        if (renderSystem->ShadersAreAvailable())
        {
            IShader *shader = renderSystem->CreateShader("HemisphereLand.vert",
"HemisphereCliffGrass.frag", video::EMT_TRANSPARENT_VERTEX_ALPHA);
            shader->SetVertexRegisterMap(get_vertex_shader_register_map());
            shader->SetPixelRegisterMap(get_pixel_shader_register_map());
...
void Level::ApplyWoodShaders(IMesh *mesh)
{
    ...
    for (u32 i = 0; i < mesh->GetMaterialCount(); i++)
    {
        ...
        if (renderSystem->ShadersAreAvailable())
        {
            IShader *shader = renderSystem->CreateShader("Hemisphere.vert",
                                                         "HemisphereWood.frag");
            shader->SetVertexRegisterMap(get_vertex_shader_register_map());
            shader->SetPixelRegisterMap(get_pixel_shader_register_map());
...
void Level::ApplyBalloonShaders(IMesh *mesh)
{
    ...
    for (u32 i = 0; i < mesh->GetMaterialCount(); i++)
    {
        ...
        {
            IShader *shader =
                renderSystem->CreateShader("HemisphereBalloon.vert",
                                           "HemisphereBalloon.frag",
...
void Level::ApplyPlainAlphaShaders(IMesh *mesh, u8 alpha)
{
    ...
    for (u32 i = 0; i < mesh->GetMaterialCount(); i++)
    {
        ...
        scene::IMeshManipulator *mm = engine->GetIrrlichtDevice()
                                          ->getSceneManager()
                                          ->getMeshManipulator();
        ...
        {
            IShader *shader =
                renderSystem->CreateShader("PlainWithAlpha.vert",
                                           "PlainWithAlpha.frag",
                                           video::EMT_TRANSPARENT_VERTEX_ALPHA);
            shader->SetVertexRegisterMap(get_vertex_shader_register_map());
            shader->SetPixelRegisterMap(get_pixel_shader_register_map());
...
IMesh *Level::AddObject(core::vector3di mapCoord, const c8 *meshName,
                        bool movable, E_OBJECT_TYPE type, bool solid)
{
    ...
    if (solid)
    {
        ...
        ICollisionGeometry *geom =
            physics->CreateBoxCollisionGeometry(core::vector3df(1.0, 1.0, 1.0));
...
void Level::MaybeAddFlower(core::vector3df absPos)
{
    ...
    if (rand()%100 < 10)
    {
        ...
        for (u32 i = 0; i < flowerMesh->GetMaterialCount(); i ++)
        {
            ...
            flowerMesh->GetMaterial(i).MaterialType =
    video::EMT_TRANSPARENT_ALPHA_CHANNEL_REF;
        }

        flowerMesh->SetPosition( absPos
                + core::vector3df(0+(rand()%1000*0.001)-0.5, 0.5,
    0+(rand()%1000*0.001)-0.5) );

        flowerMesh->SetRotation( core::vector3df(0,rand()%360,0) );

        flowerMesh->Scale(core::vector3df(1,1,1) * 0.75 + 0.01 *
    (f32)(rand()%10));
...
void Level::OptimiseLevel()
{
    ...
    // Make a list of all ground blocks which are surrounded completely by other
    // ground blocks.
    ...
    {
        // If a ground block, and surrounded completely by other ground
        // blocks...
        ...
        if (map->GetObjectType(objectCoord) == EOT_GROUND_BLOCK)
        {
            ...
            for (j = 0; j < cross.size(); j++)
            {
                ...
                if (!map->GetObject(coord) ||
                    map->GetObjectType(coord) != EOT_GROUND_BLOCK)
                    break;
            }
    ...
    for (auto &surrounded_GroundBlock : surrounded_GroundBlocks)
    {
        ...
        IMesh *mesh =
            AddObject(surrounded_GroundBlock, "ground_single_sideless.b3d",
                      false, EOT_GROUND_BLOCK);
    ...
    if (combineMeshes)
    {
        ...
        mbGrass->getMaterial().TextureLayer[0].Texture =
            driver->getTexture("grass1_lighter.jpg");
        mbMud->getMaterial().TextureLayer[0].Texture =
        ...
        for (auto &objectCoord : objectCoords)
        {
            if (map->GetObjectType(objectCoord) == EOT_GROUND_BLOCK)
            {
                ...
                // Therefore, we just remove children of this block, and not the
                // block itself. (the block itself is the physical body, and
                // needs to stay)
                ...
                scene::IMesh *otherMesh =
                    objMesh->GetIrrlichtNode()->getMesh()->getMesh(0);

                for (u32 j = 0; j < otherMesh->getMeshBufferCount(); j++)
                {
                    // We want a completely new, unique mesh buffer, so we can
                    // modify it how we like.
                    ...
                    mbTemp->append(otherMb->getVertices(),
                                   otherMb->getVertexCount(),
                                   otherMb->getIndices(),
                                   otherMb->getIndexCount());
                    ...
                    // Now adjust UVs (do what formerly had to be done via the
                    // texture matrix)
                    ...
                    io::path textureName =
                        os::path::basename(otherMb->getMaterial()
                                               .TextureLayer[0]
                                               .Texture->getName());
                    ...
                    // NOW, we combine all separate mud textures into a single
                    // buffer...

                    if (textureName == "grass1_lighter.jpg")
                    {
                        mbGrass->append(mbTemp->getVertices(),
                                        mbTemp->getVertexCount(),
                                        mbTemp->getIndices(),
                                        mbTemp->getIndexCount());
                    }
                    else // is mud
                    {
                        mbMud->append(mbTemp->getVertices(),
                                      mbTemp->getVertexCount(),
                                      mbTemp->getIndices(),
                                      mbTemp->getIndexCount());
                    }

                    // Discard the temporary buffer now contents are in the
                    // grass or mud buffers.
                    mbTemp->drop();
                }
        ...
        smgr->createMeshWriter(scene::EMWT_OBJ);

            io::IFileSystem *fileSys =
        engine->GetIrrlichtDevice()->getFileSystem(); io::IWriteFile *writeFile
        = fileSys->createAndWriteFile("test");
    ...
    for (auto &eventCoord : eventCoords)
    {
        ...
        if (map->GetEvent(eventCoord)->GetType() == EET_FAN_EVENT)
        {
                ...
                map->GetEvent(coordBelow)->GetType() != EET_FAN_EVENT)
            {
                ...
                // Now count the fan events stacked above this one, to get the
                // height for the fan particle system
                while (true)
                {
                    coord.Y += 1;

                    if (map->GetEvent(coord) &&
                        map->GetEvent(coord)->GetType() == EET_FAN_EVENT)
                        fanEventCount++;
                    else
                        break;
                }
...
void Level::AddFanParticleSystem(core::vector3di mapCoord, f32 height)
{
    ...
    ps->setPosition(mapPos +
                    core::vector3df(0, -1.0, 0)); // core::vector3df(0,-0.5,0));
    ...
    emitter = ps->createCylinderEmitter(
        ...
        0.1,                                // length
        true, core::vector3df(0, speed, 0), // velocity
        1,                                  // min per second
        4,                                  // max per second
    ...
    // rotation affector
    affector =
        ps->createRotationAffector(core::vector3df(0, -100.0, 0), mapPos);
    ps->addAffector(affector);
    affector->drop();
    ...
    ps = smgr->addParticleSystemSceneNode(false);
    ps->setPosition(
        mapPos + core::vector3df(0, -1.0, 0)); // core::vector3df(0,-0.75,0));
    ...
    emitter = ps->createCylinderEmitter(
        ...
        core::vector3df(0, 1, 0),            // normal
        0.1,                                 // length
        false, core::vector3df(0, speed, 0), // velocity
        3,                                   // min per second
        7,                                   // max per second
    ...
    // rotation affector
    affector = ps->createRotationAffector(core::vector3df(0, -25.0, 0), mapPos);
    ps->addAffector(affector);
    affector->drop();
...
void Level::CreateObject(core::vector3di mapCoord, E_OBJECT_TYPE type)
{
    ...
    case EOT_PLAYER_INTERSECTING:
        ...
        return; // must return now so coordinate is not added to level bounding
                // box (done below)
    ...
    case EOT_BALLOON:
    {
        ...
        IBobAnimator *bobAnim =
            world->CreateBobAnimator(0.04, 1.0 + (rand() % 100) * 0.002);
    ...
    case EOT_FAN:
    {
        ...
        IMotionAnimator *anim =
            world->CreateRotationAnimator(core::vector3df(0, 150.0, 0));
        ...
        // orb bob up and down..
        IBobAnimator *bobAnim =
            world->CreateBobAnimator(0.035, 1.0 + (rand() % 100) * 0.001);
    ...
    case EOT_LIFT:
    {
        ...
        // mat.TextureLayer[0].Texture =
        // engine->GetIrrlichtDevice()->getVideoDriver()->getTexture("lift_glow.png");
    ...
    case EOT_EXIT_PORTAL:
    {
        ...
        for (u32 i = 0; i < 80; i++)
        {
            ...
            core::vector3df offPos = core::vector3df(
                0, bottom + (top - bottom) * (rand() % 1000) * 0.001, 0);
            ...
            // f32 minSize = 0.6;
            // f32 maxSize = 0.9;
            // teleportMesh->Scale( core::vector3df(1,1,1) * minSize +
            // (maxSize-minSize) * (rand()%100)*0.01 );
            teleportMesh->Scale(core::vector3df(1, 1, 1) *
                                (offPos.Y + (rand() % 1000) * 0.0001));

            teleportMesh->ApplyTransformNow();

            IMotionAnimator *anim = world->CreateRotationAnimator(
                core::vector3df(0, -150.0, 0) * (1.0 + (rand() % 200) * 0.01));
...
{
    if (!logicEnabled) // hack! so blocks used as decoration in final scene are
                       // immovable
        return;

    // used below
    // bool isBalloon = map->GetObject(mapCoord) && map->GetObjectType(mapCoord)
    // == EOT_BALLOON;
    ...
    */
    ...
    if (map->ObjectMoveWillComplete(mapCoord, mapCoord + pushVec))
    {
        ...
        {
            static_cast<GridBasedCharacterController *>(playerController)
                ->EndMove();
        }
        ...
        && world->GetCameraController() && !isPlayerPushing)
    {
        if (fallen)
        {
            ...
            if (!map->GetObject(below) || map->GetObjectType(below) != EOT_LIFT)
                GetPlayer()->SetAnimations(ANIM_FALL_UPRIGHT,
                                           ANIM_FALL_UPRIGHT);
        }

        if (!fallen && !isTraversing &&
            GetPlayer()->IsSetAnimations(ANIM_FALL_UPRIGHT, ANIM_FALL_UPRIGHT))
...
{
    IProxyTransformable *proxyTransformable =
        world->AddProxyTransformable(entity);

    proxyTransformable->SetPositionOffset(
        core::vector3df(0, -(1.0 - ACTOR_PHYSICAL_HEIGHT) * 0.5, 0));
...
void Level::CreatePlayer(core::vector3di mapCoord)
{
    ...
    // NOTE: We previously set material with transparent vertex alpha here so
    player
    // can be made transparent while teleporting. However keeping that material
    all the
    // time caused glitches with the transparent sea on the final scene.
    // So instead we use the regular material here, and only set the transparent
    one
    ...
    */
    ...
    else
    {
        ...
        mesh->Translate(
            core::vector3df(0.23, 0.19,
                            -0.02)); // core::vector3df(0.1,0.19,-0.15));
    }
    ...
    // NOTE: We previously set material with transparent vertex alpha here so
    // player can be made transparent while teleporting. However keeping that
    // material all the time caused glitches with the transparent sea on the
    // final scene. So instead we use the regular material here, and only set
    // the transparent one when we touch a teleporter.
    // ApplyDefaultShaders(mesh);//, video::EMT_TRANSPARENT_VERTEX_ALPHA);
    ...
    ICharacter *entity =
        world->AddCharacter(mesh, ANIM_IDLE, ANIM_WALK, ACTOR_PHYSICAL_HEIGHT);
    ...
    physics->SetCollisionMaterialInteraction(
        material, &(physics->GetDefaultCollisionMaterial()),
        materialInteraction);

    // Add actor
    AddActor(mapCoord, entity);

    // This disabled since we want footsteps sound even when walking against a
    // wall. (in which case no motion is detected)
    /*
    IMotionSensor *motionSensor = world->AddSoundMotionSensor(
            // I need a search path manager thing.
            NULL,
...
void Level::SetMeshAlpha(IMesh *mesh, u8 alpha)
{
    scene::IMeshManipulator *mm =
        engine->GetIrrlichtDevice()->getSceneManager()->getMeshManipulator();
    mm->setVertexColorAlpha(mesh->GetIrrlichtNode()->getMesh(), alpha);
}
...
void Level::CreateMonster(core::vector3di mapCoord)
{
    ...
    ICharacter *entity = world->AddCharacter(mesh, ANIM_IDLE, ANIM_WALK,
ACTOR_PHYSICAL_HEIGHT);

    entity->SetDensity(0.4);
    entity->SetMoveSpeed(0.5);

    // Custom monster AI controller
    MonsterCharacterController *controller = new
MonsterCharacterController(this,GetPlayer()); entity->SetController(controller);
    controller->drop();
    ...
    //smgr->getMeshManipulator()->setVertexColorAlpha(mesh->GetIrrlichtNode()->getMesh()->getMesh(0),
100);
...
Level::Level(MainState *mainState, core::stringc fileName,
             std::deque<UndoState> *undoHistory)
{
    ...
    timeSinceLastUndoSave = 10000.f; // want to be able do undo immediately. (in
                                     // case undone to a difficult situation)

    NOTE << "Level::Level: " << fileName;

    // Ugly hack.
    // In some special cases we don't want grid based movement.
    if (GetShortName() == "titlescreen.lev" || GetShortName() == "intro.lev" ||
        GetShortName() == "finalscene1.lev" ||
    ...
    defaultEvent = new DefaultEvent(this);
    mapEventInstances.push_back(defaultEvent);
    ...
    else
    {
        ...
        else
        {
            ...
            for (auto &line : lines)
            {
                std::vector<core::stringc> parts =
                    str::explode_at_assignment(line);
                ...
                if (parts[0].size() && parts[1].size())
                {
                    ...
                    // finished pushing text lines, now we have the trigger
                    // condition for those lines.
                    if (parts[0] == "trigger" || parts[0] == "trigger_x" ||
                        parts[0] == "trigger_y" || parts[0] == "trigger_z" ||
                        parts[0] == "trigger_surround")
                    {
                        bool fail = false;
                        ...
                        if (parts[0] == "trigger")
                        {
                            ...
                            std::vector<core::stringc> triggerParts =
                                str::explode(",", parts[1]);
                            ...
                            {
                                WARN << "Invalid trigger in tutorial text: "
                                     << line;
                                fail = true;
                            }
                        }
            ...
            for (auto &elem : tutorialTexts)
            {
                ...
                NOTE << "Condition: trigger on map location {" << elem.trigger.X
                     << "," << elem.trigger.Y << "," << elem.trigger.Z << "}";
            }
        }
    }
    ...
    core::vector3df initVec3d =
        boundingBox.getCenter() - GetPlayer()->GetPosition();
    core::vector2df initVec2d(initVec3d.X, initVec3d.Z);
    initVec2d.normalize();
    GetPlayer()->SetRotation(
        core::vector3df(0, initVec2d.getAngle() + 90.0, 0));
    ...
    // (if I remember correctly this was formerly used for the level preview
    ...
    camera->SetPosition(boundingBox.getCenter() +
                        core::vector3df(1, 1, 1).normalize() * distance);
...
Level::~Level()
{
    ...
    for (auto &mapObject : mapObjects)
    {
        if (ITransformable *entity = map->GetObject(mapObject))
        {
            ...
            // if this was an actor, it has been removed, so remove from actors
            // list. (so we don't attempt RemoveTransformable on an actor again
            // below)
...
void Level::Start()
{
    ...
    IInputProfile *inputProfile =
        world->CreateInputProfile(6, 2); // 6 buttons and 2 axis
    ...
    bool zoomEnabled = !(GetShortName() == "first.lev" ||
                         GetShortName() == "shiftblocks.lev" ||
    ...
    thirdPersonCamera =
        world->CreateThirdPersonCameraController(defaultZoomDistance, 0.0,
                                                 -85.0, 85.0);
    ...
    thirdPersonCamera->SetZoomLimits(1.5,
                                     max(boundingBox.getExtent().getLength(),
                                         10.f));
    ...
    if (localGridBasedMovement)
    {
        // Custom controler that only allows movement in the 4 compass
        // directions.
        playerController =
            new GridBasedCharacterController(world, 0, 1, 2, 3,
                                             world->GetCamera(), this);
    }
    else
    {
        playerController =
            world->CreateUserCharacterController(0, 1, 2, 3,
                                                 world->GetCamera());
    }
    ...
    // thirdPersonCamera->CreateThirdPersonCameraCollider();
...
void Level::ReplaceWith(core::stringc fileName)
{
    ...
    core::vector3df oldVec =
        (camera->GetPosition() - boundingBox.getCenter()).normalize();
...
void Level::OnEvent(const Event &event)
{
    ...
    if (event.IsType("PlayerPushedMove"))
    {
        ...
        bool isBalloon = map->GetObject(pushStartLoc) &&
        map->GetObjectType(pushStartLoc) == EOT_BALLOON;
...
UndoState Level::CreateUndoState()
{
    ...
    for (auto &coord : mapLocations)
    {
        ...
        mapLoc.eventType = map->GetEvent(coord)
...
void Level::ApplyUndo(bool died)
{
    ...
    if (mainState)
    {
        if (ExistsUndo())
        {
            // Through all past states, incrementing undo and death counters as
            // necessary. Since those things remain even after undoing.
...
void Level::Load(UndoState *undoState)
{
    ...
    else
    {
        ...
        for (auto &mapContent : mapContents)
        {
            ...
            // for undostate, we ingore all player start events, instead using
            // the player's centre location as the start position
                ...
                (E_OBJECT_TYPE)loc.objectType != EOT_PLAYER_INTERSECTING)
            {
                if (map->GetObject(loc.coord))
                    ...
                    WARN << "error with undo: previous coord set but no object "
                            "was created!";
            }
        ...
        // not the current BB.
        // (since a block may have been pushed off the world, making the BB very
        // big, and player will fall forever)
    ...
    {
        GetPlayerActor().entity->SetPosition(
            GetPosFromCoord(startPositions[rand() % startPositions.size()]));
...
bool Level::StartMapObjectMove(core::vector3di source, core::vector3di dest,
                               bool forceMove)
{
    if (map->StartObjectMove(source, dest, forceMove))
    {
        // Check if a actor is being moved, if so then update the actor's
        // centreLocation.
        for (auto &c : actors)
        {
...
void Level::StartNextLevelTransition(bool stopMoving)
{
    ...
    if (name == "intro.lev" || name == "finalscene1.lev" ||
        name == "finalscene2.lev")
    {
        // Fade off
...
void Level::TouchedEndLevelPortal(ITransformable *portal)
{
    ...
    for (u32 i = 0; i < 40; i++)
    {
        ...
        mesh->SetPosition(
            GetPlayer()->GetAbsolutePosition() +
            core::vector3df(
                0, -0.5 + bottom + (top - bottom) * (rand() % 100) * 0.01, 0));

        f32 minSize = 0.45;
        f32 maxSize = 0.65;
        mesh->Scale(core::vector3df(1, 1, 1) * minSize +
                    (maxSize - minSize) * (rand() % 100) * 0.01);
        ...
        // (this is because now level doesn't always fade off, we sometimes see
        // the end level screen + scores)
...
public:
    ...
    {
        // (waitTime has passed AND a certain distance has been moved by the
        // player) OR (a next tutorial item is queued and waiting) OR (mouse
        // left is down)
        return (eventQueue->GetVirtualTime() > waitStartTime + displayTime &&
                level->GetPlayer()->GetPosition().getDistanceFrom(
                    originalPlayerPos) > 1.5f) ||
...
void Level::Update(f32 dt)
{
    ...
    if (logicEnabled)
    {
        ...
        if (tutorialTexts.size())
        {
            for (u32 i = 0; i < tutorialTexts.size(); i++)
            {
                ...
                case TTT_SURROUND:
                {
                    ...
                    surrounding.push_back(tt.trigger +
                                          core::vector3di(1, 0, 1));
                    surrounding.push_back(tt.trigger +
                                          core::vector3di(1, 0, 0));
                    surrounding.push_back(tt.trigger +
                                          core::vector3di(1, 0, -1));
                    surrounding.push_back(tt.trigger +
                                          core::vector3di(0, 0, -1));
                    surrounding.push_back(tt.trigger +
                                          core::vector3di(-1, 0, -1));
                    surrounding.push_back(tt.trigger +
                                          core::vector3di(-1, 0, 0));
                ...
                // We just queue stuff here, the actual work is done when events
                // are received in OnEvent.
                if (isHit)
                {
                    ...
                    // Logic wait. More complex logic that decides when the
                    // tutorial text should be removed. (based on time and
                    // distance moved by player)
                    IWaitLogic *waitLogic =
                        new TutorialWait(this, tt.displayTime);
                    ...
                    // More tutorial texts can then follow on from this in the
                    // queue.
                    {
                    ...
                    // We've created all necessary events, so remove tutorial
                    // item from list.
        ...
        if (map->GetObject(playerMapPos) &&
            map->GetObjectType(playerMapPos) == EOT_EXIT_PORTAL)
        {
            if (!touchedEndLevelPortal)
            {
                TouchedEndLevelPortal(map->GetObject(playerMapPos));

                // Change player to a transparent vertex alpha material so it
                // can be faded out. (we don't use this material all the time as
                // it does not go well with semi transparent sea on the final
                // scene)
                ApplyDefaultShaders(GetPlayer()->GetMesh(),
                                    video::EMT_TRANSPARENT_VERTEX_ALPHA);
            }
        }
        ...
        if (touchedEndLevelPortal)
        {
            ...
            f32 alpha =
                (world->GetUpdater().GetVirtualTime() - portalTouchedTime) /
                2.0;
        ...
        if (playerPos.Y < lowestPoint)
        {
            ...
            core::vector3df camToPlayer =
                (GetPlayer()->GetPosition() - world->GetCamera()->GetPosition())
                    .normalize();
            ...
            if (!fallCameraMoved)
            {
                ...
                core::line3df ray(GetPlayer()->GetPosition(),
                                  world->GetCamera()->GetPosition());

                // exclude player from collision
                Set<ICollisionGeometry *> excludedGeometries =
                    GetPlayer()->GetBody()->GetCollisionGeometries();

                RayCollision collisionResult;

                if (physics->RayCastExcluding(ray, excludedGeometries,
                                              &collisionResult))
                {
                    world->GetCamera()->SetPosition(
                        collisionResult.collision.pos -
    ...
    // fall animation!? for falling short distances when will land on another
    // platform. (not fallen into space)
    /*
    {
        ...
        for (u32 i = 0; i < 4; i ++)
        {
            ...
            if (map->GetObject(belowPlayer))
            {
                ...
                if (objectType == EOT_PLAYER_CENTRE || objectType ==
    EOT_PLAYER_INTERSECTING
        ...
        // But before switching animation back from fall anim we would want a
    minimum fall anim time...
        // Could we just generate an event here?
        // And set the animation back when the event is received?
        // (i.e. a set time in the future)
        // would need to ensure it didn't interfere with other fall
    animations...
    ...
    */
    ...
    /*
    if (world->GetCameraController() && !isPlayerPushing // not falling off
    world AND not pushing?
            && (!map->GetEvent(playerMapPos) ||
    map->GetEvent(playerMapPos)->GetType() != EET_FAN_EVENT)
            )
    {
        // Analyse space below player.
    ...
    */
    ...
    // is either a balloon and was not previously, or is no longer a balloon and
    // was previously
    bool changedFootStepSoundType = (objectBelowPlayer == EOT_BALLOON &&
                                     lastObjectBelowPlayer != EOT_BALLOON) ||
                                    (objectBelowPlayer != EOT_BALLOON &&
                                     lastObjectBelowPlayer == EOT_BALLOON);
    ...
    if (localGridBasedMovement)
    {
        ...
        if (GetPlayer()->GetAttemptedMove().getLengthSQ() > 0.001)
        {
            ...
            if (fabs(GetPlayer()->GetAttemptedMove().X) >
                fabs(GetPlayer()->GetAttemptedMove().Z))
            {
    ...
    // if we've touched the end level portal we don't want to interfere with the
    // visibility state (since touching portal may make character invisible)
    if (!touchedEndLevelPortal)
    {
        // hide player if camera is closer than a certain distance
        GetPlayer()->GetMesh()->SetVisible(
            world->GetCamera()->GetTargetDistance() > 0.5
    ...
    // if so, and if there is a moveable block in that square, movable block
    // should move.
    ...
    // Probably want to first check if any of these actually have a movable
    // block in them.
    ...
    for (auto &adjacentCros : adjacentCross)
    {
        ...
        if (map->GetObject(adjacentCros)
            ...
            && IsActorTouchingCoord(GetPlayerActor(), adjacentCros,
                                    1.05) // 1.8)//1.3)// was 1.05
            // Also mustn't push if this is the player's own location!?
            && (!GetPlayerActor().centreLocationSet ||
                adjacentCros != GetPlayerActor().centreLocation)
            // ...and mustn't push other actors (enemies)
            && !(map->GetObject(adjacentCros) &&
                 map->GetObjectType(adjacentCros) == EOT_PLAYER_CENTRE)
            // And mustn't push if there is a movable object above this one!
            // UPDATE: commented out movable check, now cannot move if any
            // object is above
            //&& !(map->GetObject(above) /*&& map->IsObjectMovable(above)*/)
            // UPDATE2: Now using defaultEvent for this.
            && defaultEvent->RequestActionPermission(adjacentCros,
                                                     EAT_MOVE_BY_PUSH))
        {
            // BOOYA. Touching.
            ...
            // But also, need to check that the player has moved (in XZ plane)
            // towards this object (from the last update) rather than away from
            // it. (don't push if moving away or no move).
            ...
            core::vector3df attemptedMove =
                GetPlayer()->GetAttemptedMove() * 0.1;
            core::vector2df currentPlayerPos2D(playerPos.X, playerPos.Z);
            core::vector2df attemptedMove2D(attemptedMove.X, attemptedMove.Z);
            core::vector2df desiredPlayerPos2D =
                currentPlayerPos2D + attemptedMove2D;
            core::vector2df coordPos(GetPosFromCoord(adjacentCros).X,
                                     GetPosFromCoord(adjacentCros).Z);

            if (coordPos.getDistanceFromSQ(desiredPlayerPos2D) + 0.001 <
                coordPos.getDistanceFromSQ(currentPlayerPos2D))
            {
                // ALSO, NEW UPDATE: Only push if the player is actually
                // *facing* the block to be moved. We don't want grazing contact
                // to cause a move (even if the player is actually touching and
                // closer) touchDist in the direction of attemptedMove should be
                // within some distance of the block's centre.
                ...
                if ((currentPlayerPos2D + attemptedMove2D * touchDist)
                        .getDistanceFrom(coordPos) < someDistance)
                {
                    // Vector to move the movable block (if possible and next
                    // map coord is unoccupied)
    ...
    // Determine and take ownership (if possible) of any squares an actor
    // intersects. Similar to above, but using the surrounding eight squares.
    // Also attempt to set or move the actor centre location, which is a movable
    // block.
    ...
    for (auto &c : actors)
    {
        for (u32 j = 0; j < c.lastLocations.size(); j++)
        {
            ...
            // (only way it wouldn't is if SetObject called elsewhere, or
            // moveobject called with forceMove)
    ...
    // Attempt to move any actor location that the actor has moved to a
    // different location. If the move fails then it will stay where it is...
    for (auto &c : actors)
    {
        core::vector3di actorMapPos = GetCoord(c.entity->GetPosition());
        ...
        if (c.centreLocationSet && actorMapPos != c.centreLocation)
        {
            ...
            // So maybe cache an undo state, this will only be saved if
            // something bad happens as a result of the following move.
            if (map->ObjectMoveWillComplete(c.centreLocation, actorMapPos,
                                            false))
            {
                ...
                while (true)
                {
                    ...
                    if (checkType == EOT_GROUND_BLOCK ||
                        checkType == EOT_LIFT || checkType == EOT_BALLOON ||
                        checkType == EOT_GROUND_BLOCK_FALL)
                    {
                ...
                if (wasOnSolidGround)
                {
                    core::vector3di below =
                        actorMapPos + core::vector3di(0, -1, 0);

                    // Save for Undo if just fallen!
                    // But only once, when first fall is detected.
                    if (objectBelowPlayer == EOT_UNKNOWN &&
                        lastObjectBelowPlayer != EOT_UNKNOWN)
                    {
                    ...
                    else if (map->GetEvent(actorMapPos) &&
                             map->GetEvent(actorMapPos)->GetType() ==
                                 EET_FAN_EVENT &&
                             (!map->GetEvent(c.centreLocation) ||
                              map->GetEvent(c.centreLocation)->GetType() !=
                                  EET_FAN_EVENT))
                    {
                        willSaveUndoState = true;
                    ...
                    // or we are on a lift, and weren't previously on the *SAME*
                    // lift (but we *do* save before stepping from one lift to a
                    // new one) if X and Y are not the same as before, we've
                    // moved in the ground plane so are on a different lift.
                    else if (map->GetObject(below) &&
                             map->GetObjectType(below) == EOT_LIFT &&
                             !(actorMapPos.X == c.centreLocation.X &&
                               actorMapPos.Z == c.centreLocation.Z))
                    {
                        willSaveUndoState = true;
                    }
                    // or we are just moved on to an exploding block
                    else if (map->GetObject(below) &&
                             map->GetObjectType(below) == EOT_GROUND_BLOCK_FALL)
                    {
                        willSaveUndoState = true;
                    }
                }
            ...
            // If the destination contains an event, and that event DENIES
            // LOCOMOTION, this is a special case and we want to do an
            // interpolated move into that square.

            // if (map->GetEvent(actorMapPos)
            //		&&
            //! map->GetEvent(actorMapPos)->RequestActionPermission(actorMapPos,
            // EAT_LOCOMOTE))
            if (!defaultEvent->RequestActionPermission(actorMapPos,
                                                       EAT_LOCOMOTE))
            {
            ...
            {
                // Normal behaviour.
                // Actor takes control of square without interpolated movement
                // into the centre.
    ...
    for (auto &c : actors)
    {
        ...
        std::vector<core::vector3di> possibleActorLocations =
            GetActorIntersectingCoords(c);
        ...
        for (auto &possibleActorLocation : possibleActorLocations)
        {
            ...
            if (!map->GetObject(loc)
                ...
                // FIX:
                // Also don't occupy this location if we just pushed an object
                // out from there. (since there may be a stack above that ought
                // to fall into this location)
    ...
    // If actor is on an event square and that event denies gravity... then
    // disable gravity! Otherwise enabled it. Maybe should also zero downwards
    // velocity...
    for (auto &c : actors)
    {
        core::vector3di actorMapPos = GetCoord(c.entity->GetPosition());

        c.GetCharacter()->SetGravityEnabled(
            defaultEvent->RequestActionPermission(actorMapPos, EAT_FALL));

        // Also check for locomotion here. if locomotion is not allowed, then
        // reposition actor on square centre.
        ...
        {
            c.GetCharacter()->SetGravityEnabled(
        event->RequestActionPermission(actorMapPos, EAT_FALL) );

            // Also check for locomotion here. if locomotion is not allowed,
        then reposition actor
            // on square centre.

            if ( !event->RequestActionPermission(actorMapPos, EAT_LOCOMOTE)
                    // Only deny locomotion if player is actually exactly on a
        location.
                    // (i.e. they have finished traversing into it)
                    && !map->IsTraversing(actorMapPos)
                    //&& c.entity->GetPosition() == GetPosFromCoord(actorMapPos)
    ...
    if (logicEnabled) // But don't do this if in editor.
    {
        ...
        // Called when an object has completely finished moving in to a new
        // position.
        for (auto &coord : locationsEntered)
        {
            ...
            // e.g. falling.
            // MUST ALSO ASK EVENT OWNER FOR PERMISSION TO PERFORM THE ACTION,
            // IF EVENT IS SET. event->RequestActionPermission(...)

            ITransformable *object = map->GetObject(coord);

            // Call event if set
            // Called before gravity etc, as gravity may move the object,
            // skipping event.

            if (event)
                event->OnMapEvent(coord);

            // Only fall if the object has not moved/changed as a result of the
            // event
        ...
        // Update: Gravity is also done by brute force here. This is so gravity
        // is applied when no location is entered (i.e. the object underneath
        // this one disappears). Gravity application in locationsEntered above
        // is probably no longer needed, but has been left just in case.
...
void Level::ShovePlayerOutOfTheWay(core::vector3di coord)
{
    ...
    for (auto &c : actors)
    {
        ...
        {
            // Found the actor, now find its record of this coord (in
            // lastLocations).
            for (u32 j = 0; j < c.lastLocations.size(); j++)
            {
                ...
                if (c.lastLocations[j] == coord)
                {
                    ...
                    // (same as when refreshing actor occupied locations in
                    // Update)
                    map->SetObject(c.lastLocations[j], nullptr, false,
                                   EOT_UNKNOWN);
