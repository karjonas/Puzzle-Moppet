...
public:
    ...
    // This should be called after SetPosition/SetRotation in order to position
    // an object immediately. (i.e. when positioning initially at the start of a
    // game or when teleporting to a new location).
    ...
    ITransformable *GetParent() { return parent; }

    // Get all children which are a specific class.
    template <class Type>
    std::vector<Type *> GetChildrenByType()
    {
    ...
    // Requiring the scene graph to be enabled is really just a hack to avoid
    // having to pass the world pointer through all derived class constructors.
    void EnableSceneGraph(IWorld *world) { this->world = world; }

    // ************** Transformation **********
    // Rotations are euler angles in degrees.

    virtual void SetPosition(const core::vector3df &pos) { relativePos = pos; }

    virtual void SetRotation(const core::vector3df &rot) { relativeRot = rot; }

    virtual core::vector3df GetPosition() { return relativePos; }

    virtual core::vector3df GetRotation() { return relativeRot; }
    ...
    core::vector3df GetAbsoluteRotation()
    {
        if (parent)
        {
            ...
            // return combine_rotations(parent->GetAbsoluteRotation(),
            // GetRotation());
        }
        else
            return GetRotation();
    ...
    const std::vector<IMotionAnimator *> &GetAnimators() { return animators; }

    // **************** Velocity utilities ***************

    // There is a lag of one frame (the velocity given is the velocity of the
    // previous frame). This is required because GetLinearVelocity might be
    // called *after* the old position has been cached but before a new position
    // is calculated (which would always yield a velocity of zero). On the first
    // and second frames after calling ApplyTransformNow() the velocity will be
    // zero. NOTE: There is no GetAngularVelocity method as it is impossible to
    // calculate simply by observing the rotation: rotations wrap around so you
    // cannot know how many revolutions have occurred.
    ...
    core::vector3df GetCachedPosition() { return cachedPos; }

    core::vector3df GetCachedRotation() { return cachedRot; }
    ...
    // Interpolates from a cached transformation (cached at 2nd-to-last logic
    // update) to the current transformation (as set in the last logic update).
    // This can be overridden to give more complicated interpolation.
    // (for example, perfect camera movement at a low logic FPS requires
    // interpolation in polar coordinates around a target position)

    virtual core::vector3df GetInterpolatedPosition(f32 alpha)
    {
        return maths::interpolate_position(GetCachedPosition(), GetPosition(),
                                           alpha);
    }

    virtual core::vector3df GetInterpolatedRotation(f32 alpha)
    {
        return maths::interpolate_rotation(GetCachedRotation(), GetRotation(),
                                           alpha);
    }
    ...
    core::vector3df GetInterpolatedAbsolutePosition(f32 alpha)
    {
        ...
        {
            core::matrix4 parentMat;
            parentMat.setTranslation(
                parent->GetInterpolatedAbsolutePosition(alpha));
            parentMat.setRotationDegrees(
                parent->GetInterpolatedAbsoluteRotation(alpha));
            core::matrix4 mat;
            mat.setTranslation(GetInterpolatedPosition(alpha));
            mat.setRotationDegrees(GetInterpolatedRotation(alpha));
            core::matrix4 absMat = parentMat * mat;
            return absMat.getTranslation();
            // return parent->GetInterpolatedAbsolutePosition(alpha) +
    ...
    core::vector3df GetInterpolatedAbsoluteRotation(f32 alpha)
    {
        if (parent)
        {
            ...
            return absMat.getRotationDegrees();
            // return
            // combine_rotations(parent->GetInterpolatedAbsoluteRotation(alpha),
