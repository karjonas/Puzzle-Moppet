...
// Tasks are all created internally, at the same time (engine start), so that
// their virtual times are in sync. (all start at time=0 on engine start) So
// task's virtual time is the same as Engine time.
...
protected:
    // Pause and resume are not available for the top level tasks, since they
    // mustn't get out of sync.
...
public:
    ...
    // (there's no real need for unlimited FPS rendering is there? We don't want
    // to be rendering at 1000 FPS on later hardware) So for a fixed timestep,
    // all updates are done and none are skipped. For a variable timestep, some
    // updates may be skipped if there is not enough time.
    ...
    virtual void Update(f32 dt) override { IUpdatable::Update(dt); }

    // Gets the interpolation from this task's previous Update to the current
    // point in time. Note: this is only useful if this task has a fixed time
    // step This is used by engine.GetLogicInterpolationAlpha()
...
private:
    ...
    // If the alotted timestep has passed, or if this task is not at a fixed
    // time step, then Update() is called.
