...
public:
    GUIPane(gui::IGUIElement *parent, core::recti rect, video::SColor col)
        : gui::IGUIElement(
              gui::EGUIET_ELEMENT,
              GetEngine()->GetIrrlichtDevice()->getGUIEnvironment(), parent, -1,
              rect)
        , col(col)
    {
    }

    void setCol(const video::SColor &col) { this->col = col; }

    video::SColor getCol() { return col; }
    ...
    void draw() override
    {
        if (IsVisible)
        {
            ...
            skin->draw2DRectangle(this, col, AbsoluteRect,
...
public:
    ...
    GUIElementFade(gui::IGUIEnvironment *env, gui::IGUIElement *parent,
                   IUpdatable *updatable, f32 lifeTime, f32 fadeTime,
                   bool fadeOut)
        : IGUIElement(gui::EGUIET_ELEMENT, env, parent, -1,
                      core::recti(0, 0, 1, 1))
        , fadeOut(fadeOut)
    {
    ...
    void OnPostRender(u32 timeMs) override
    {
        ...
        if (Parent && t >= startFadeTime)
        {
            ...
            // interpolate
            current = current.getInterpolated(video::SColor(0, current.getRed(),
                                                            current.getBlue(),
                                                            current.getGreen()),
                                              alpha);
