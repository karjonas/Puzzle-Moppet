...
public:
    ...
    // The sound has its position, velocity, etc set internally by the sound
    // source.
    virtual ISound3D *GetSound() = 0;

    // A more advanced way of playing sounds.
    // The multiple ISounds of the sound queue have their position, velocity etc
    // set by the sound source.
    virtual ISoundQueue *GetSoundQueue() = 0;

    // Add a new sound to this source.
    // If deleteOnFinish is set, then the sound will be removed after it has
    // been played once.
