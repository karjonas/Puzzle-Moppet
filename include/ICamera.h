...
/*
Camera can have position, rotation etc set. Camera looks in direction of
rotation. The camera rotation can also be set in a conventional manner using
SetTarget. Optionally, camera can be defined to interpolate smoothly around the
target with methods SetOrbitTarget and SetTargetDistance.
*/
...
public:
    ...
    // If this is other than zero, rotation occurs around this point during
    // interpolation. NOTE: Currently distance itself is not interpolated, so
    // zooming will not be smooth.
