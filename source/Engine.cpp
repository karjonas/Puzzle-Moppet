...
io::path Engine::GetSettingsPath()
{
    ...
    ASSERT(initSettings.count("appName"));
    return os::path::concat(GetLocalSettingsDir(),
                            initSettings["appName"] + ".ini");
}
...
Engine::Engine(int argc, const char **argv, const VariantMap *settings)
{
    ...
    // And then, any settings set in the config file will override everything
    // else. Since appName is used here, that is the one thing that cannot be
    // specified by the end user in the config file. (well it *can* be, but
    // don't tell the user that..)
    ...
    // Cache the original starting directory before the engine changes
    // elsewhere.
    startDir = os::getcwd();

    // Find the exe path
    // We use getexecutablepath instead of argv[0] as we want a full absolute
    // path. Also getexecutablepath resolves symbolic links on Linux.
    ...
    // Query current desktop resolution...
    {
        ...
        if (device)
        {
            cachedDesktopResolution =
                device->getVideoModeList()->getDesktopResolution();

            NOTE << "Cached original resolution: "
                 << cachedDesktopResolution.Width << "x"
                 << cachedDesktopResolution.Height;
    ...
    /*device = createDevice(initSettings["softwareMode"] ?
       video::EDT_BURNINGSVIDEO : video::EDT_OPENGL,
        core::dimension2du(initSettings["screenWidth"],
       initSettings["screenHeight"]), 32, initSettings["fullScreen"], false,
        initSettings["vsync"],
        this);*/

    SIrrlichtCreationParameters deviceParams;

    deviceParams.DriverType = initSettings["softwareMode"]
    ...
    // seemed to break video mode listing)
    // deviceParams.DeviceType = initSettings["softwareMode"] ? EIDT_BEST :
    // EIDT_SDL;
#endif
    ...
    // may vary) We call chdir here after logging has been set up, since chdir
    // will output a log message when the working directory is changed. This is
    // also moved below all calls to createDevice, since the Mac implementation
    ...
    if (!initSettings["fullScreen"])
    {
        ...
        // Linux
        // PROBLEM: Obviously cannot get video data "OpenGLLinux" when using
        // software renderer... So cannot centre window...
        ...
        (Window)videoData.OpenGLLinux.X11Window, 0, 0);
        */
#endif
    }
    ...
    device->getSceneManager()->getParameters()->setAttribute(
        scene::B3D_LOADER_IGNORE_MIPMAP_FLAG, true);

    // this light and camera scene node is temporary!
    scene::ILightSceneNode *light =
        device->getSceneManager()->addLightSceneNode(nullptr,
    ...
    scene::ICameraSceneNode *camera =
        device->getSceneManager()->addCameraSceneNode(); // FPS(0, 100.f, 50.f);
    ...
    renderTask->SetSteppingParams(false,
                                  1.0 / initSettings["maxRenderFPS"].To<f32>());
    ...
    // Post processing?
    // Although this probably wouldn't work without shaders as PP is generally
    // done with shaders.
...
Engine::~Engine()
{
    ...
    // However the logic and render tasks are not destructed yet since we kept a
    // reference to them.
    ...
    // probably ought to move this to below dropping of tasks as well (since
    // they might have sounds...)
    soundSystem->drop();

    // Tasks destructed after everything else.
    // Since at least the World may contain a logic IUpdatable (Sound Source
    // with a SoundQueue), and so will call logicTask->RemoveUpdatable on
    // destruction.
    logicTask->drop();
    renderTask->drop();
...
{
    // We use the *real* system time, and subtract the time when starting. This
    // gives us a virtual time that starts at zero when the Engine starts. We
    // don't use Irrlicht's virtual timer, because that needs to be stopped
    // sometimes (e.g. to pause graphics).
...
void Engine::RegisterEventInterest(IWantEvents *receiver,
                                   const core::stringc &eventName)
{
    specificEventInterest[eventName].Insert(receiver);
}

void Engine::UnregisterEventInterest(IWantEvents *receiver,
                                     const core::stringc &eventName)
{
...
bool Engine::OnEvent(const SEvent &irrEvent)
{
    ...
    if (irrEvent.EventType == EET_MOUSE_INPUT_EVENT)
    {
        ...
        {
            // I think there was some bug once with values from the irrEvent
            // structure, so we query the mouse ourselves.
            ...
            // we call getRelativePosition *again* here, in case any mouse axes
            // were centred above.
            lastMousePos = curcon->getRelativePosition();

#else
            // On Mac OS we do this a bit differently.
            // It seems the cursor position isn't immediately updated or
            // something, so lastMousePos does not get correctly updated with
            // the centred position. This approach would probably work on
            // Windows and Linux too, but I haven't tested yet...
