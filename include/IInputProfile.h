...
// This maps "virtual buttons" to real buttons. A real button can be a key or a
// mouse button. So a virtual button can have several real buttons mapped to it.
// e.g. a virtual button for moving the player forwards might have both the up
// key and W key mapped to it.
...
public:
    ...
    // delta is the amount (positive or negative) the axis changes in response
    // to a single button press. This allows a button to be used where an axis
    // is normally used. e.g. using buttons to control the camera rotation or
    // zoom. You could bind left arrow with delta  -1.0 and right arrow with
    // +1.0
