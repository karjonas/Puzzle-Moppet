...
bool is_executable(const io::path &path)
{
    ...
    // Comparing FindExecutable to the original path would perhaps be a better
    // way.
    ...
    // I tried originally to use stat because of this quote for access(): "an
    // implementation may indicate success for X_OK even if none of the execute
    // file permission bits are set" However I gave up; I can't find any
    // documents describing the exact order of precedence for the execute
    // permissions.
    /*
    struct stat status;
    ...
    // If we are root and the file is executable by either user, group, or
    other. if (geteuid() == 0) return status.st_mode &
    ...
    // If the current user is the file owner, and the file is not user
    executable. if (status.st_uid == geteuid() && !(status.st_mode & S_IXUSR))
        return false;

    // If the current user's group is the file's group, and the file is not
    group executable. if (status.st_gid == getegid() && !(status.st_mode &
    S_IXGRP)) return false;
