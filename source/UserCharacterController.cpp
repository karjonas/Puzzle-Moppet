...
UserCharacterController::UserCharacterController(
    IWorld *world, int idButtonWalkForwards, int idButtonWalkBackwards,
    int idButtonWalkLeft, int idButtonWalkRight, ITransformable *moveRelativeTo)
{
    this->world = world;
...
void UserCharacterController::Update(ICharacter *character)
{
    if (moveRelativeTo)
    {
        character->SetMoveVec(
            maths::get_horizontal_vec(moveRelativeTo->GetRotation()));
    }
