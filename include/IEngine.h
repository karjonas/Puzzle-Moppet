...
public:
    ...
    // This is an IUpdatable that allows more complex queueing of multiple
    // events than simply calling IEngine.QueueEvent.
    virtual IEventQueue *CreateEventQueue() = 0;

    // Zero any time deltas. Useful after finishing some loading or other, to
    // prevent a logic hang or rendering jump when rendering or logic tries to
    // catch up. Calls InitUpdateTime on the internal logic and render
    // updatables.
    ...
    // Will remain overridden until a new button input event is received that
    // changes the state. ButtonUp/Down events are generated if this is a change
    // in state.
    virtual void SetButtonState(s32 button, bool state) = 0;

    // Gets engine absolute time. This starts from zero when the engine starts,
    // and is paused when the window is minimised etc. (unless multiplayer, in
    // which case the engine will not pause when minimised) This time is
    // generally only used internally, for most purposes you should use GetTime
    // of an IUpdatable.
    virtual f32 GetEngineTime() = 0;

    // Returns the fraction that the current time is between the last and the
    // next logic steps. Used internally for rendering interpolated states
    // between logic steps.
    ...
    // Will attempt to use any command line arguments originally passed to
    // CreateEngine.
    virtual void Restart() = 0;

    // Has Restart been called? (i.e. the app will re-launch as soon as Engine
    // destructs)
    ...
    // (i.e. before the engine has changed to the binary directory, which it
    // does on init)
    ...
    // Generally this will be somewhere in the user's home directory in a sub
    // dir appName as set in EngineSettings. In Windows, this might be something
    // like C:\Documents and Settings\User\Application Data\appName In Linux,
    // might be /home/user/.appName WARNING: This probably doesn't work on non
    // ASCII systems... :S
    ...
    // Register interest in a named event. Must unregister with
    // UnregisterEventInterest or UnregisterAllEventInterest when events are no
    // longer needed.
    virtual void RegisterEventInterest(IWantEvents *receiver,
                                       const core::stringc &eventName) = 0;

    // Unregister interest in a named event.
    virtual void UnregisterEventInterest(IWantEvents *receiver,
                                         const core::stringc &eventName) = 0;
    ...
    // If enabled for an axis, that axis is centred after each move of the
    // mouse. This is useful when the mouse is being used to control a camera,
    // otherwise the pointer would reach the edge of the screen and no further
    // movement would be possible. This is internally enabled and disabled by
    // World.Resume and World.Pause respectively.
    ...
    // This is different from the Irrlicht getDesktopResolution, in that this
    // function returns the resolution that was set *before* this app was
    // started. (Irrlicht will return the new resolution if it is running full
    // screen).
    virtual core::dimension2du GetDesktopResolution() = 0;

    // Get settings used for engine creation.
    // This will be settings loaded from file, and also will contain default
    // settings.
    virtual VariantMap GetCreationSettings() = 0;

    // Get the built in default engine settings, those that are used by the
    // engine if none are passed to CreateEngine. This is possibly useful for
    // the config app but has little use elsewhere.
...
(if a key is not defined, a built in default value will be used)
    ...
    appName - Name of the application. Used in various places, such as the name
of the user data directory. Defaults to "Litha Engine".
    ...
    fullScreen - boolean, should the app run in full screen? Otherwise app is
run windowed. Defaults to false, with app running in windowed mode.

    softwareMode - should software rendering be used?
                Defaults to false, OpenGL is used.
*/
IEngine *CreateEngine(int argc, const char **argv,
                      const VariantMap *settings = nullptr);

// Get a pointer to the created engine. May only be called after engine
// creation. Useful to avoid passing pointers to the engine around everywhere.
IEngine *GetEngine();
