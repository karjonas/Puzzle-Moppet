...
public:
    ...
    // (as scale is considered permanent, unlike in Irrlicht where the BB is
    ...
    // Note that the mesh specified here should have the same number of
    // materials as the original mesh. Mesh geometry is ignored (pass will be
    // rendered with the geometry of the original mesh). (Irrlicht doesn't
    // really have a separate materials file format so we just use a mesh)
    virtual void AddPass(const c8 *meshName) = 0;

    // an animation is: {start,end,FPS,looped?,interruptible?}
    // id is chosen by you. Sensible to use an enum, e.g. enum {
    // IDLE,WALK,etc...} NOTE: Some artists may assume animation starts at 1,
    // but Irrlicht assumes it starts at 0. (so you may need to subtract 1 from
    // animation ranges)
    virtual void DefineAnimation(s32 id, s32 start, s32 end, f32 fps,
                                 bool looped, bool interruptible,
                                 f32 transitionTime = 0.1) = 0;

    // Set an animation defined with DefineAnimation.
    // This may fail if an animation that cannot be interrupted is currently
    // playing. If the animation specified is already being played nothing will
    // happen.
    ...
    // NOTE: This is not yet taken into account when generating physics
    // geometry. It's on the TODO list.
    ...
    // ESPECIALLY do not modify, add, or remove the node's materials. Material
    // modifications must be done through the IGraphic API.
