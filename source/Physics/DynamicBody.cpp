                                          ...
                                          ICollisionGeometry *geom)
{
    ...
    {
        WARN << "Only built in collision geometries may be used. Will default "
                "to a unit sphere.";
...
void DynamicBody::ApplyTorque(const core::vector3df &torque)
{
    ...
    // Z is negated. It seems to help. Want +ve Z rotation to roll in the +ve X
    // direction.
...
void DynamicBody::ZeroLinearVelocity(s32 axes)
{
    ...
    else
    {
        ...
        dBodySetLinearVel(body, axes & AXIS_X ? 0 : vel[0],
                          axes & AXIS_Y ? 0 : vel[1],
                          axes & AXIS_Z ? 0 : vel[2]);
    }
}
