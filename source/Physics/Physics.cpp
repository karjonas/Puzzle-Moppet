...
Physics::Physics(IWorld *lithaWorld)
{
    ...
    SetCollisionMaterialInteraction(defaultMaterial, defaultMaterial,
                                    defaultInteraction);
}
...
void Physics::SetCollisionMaterialInteraction(
...
CollisionMaterialInteraction *Physics::GetCollisionMaterialInteraction(
    ICollisionMaterial *m1, ICollisionMaterial *m2)
{
    if (materialInteractions.count(m1))
    {
        std::map<ICollisionMaterial *, CollisionMaterialInteraction> &sub =
            materialInteractions[m1];
...
IBoxCollisionGeometry *Physics::CreateBoxCollisionGeometry(
...
ISphereCollisionGeometry *Physics::CreateSphereCollisionGeometryFromBB(
    IMesh *mesh, bool inside)
{
...
bool Physics::GetLayerCollisions(u32 layer1, u32 layer2)
{
    if (layer1 == layer2)
        return true;
    else if (layerCollisions.count(layer1) &&
             layerCollisions[layer1].count(layer2))
...
{
    // Collide ray
    const std::vector<RayCollision> &collisions =
        ray_cast(ray, space, includingGeometry, excludingGeometry, layer, this);
...
std::vector<RayCollision> Physics::RayCastIncluding(
    const core::line3df &ray,
...
std::vector<RayCollision> Physics::RayCastExcluding(
    const core::line3df &ray,
...
void Physics::ODE_GeomCollide(dGeomID o1, dGeomID o2)
{
    ...
    if (!GetLayerCollisions(collisionGeom1->GetCollisionLayer(),
                            collisionGeom2->GetCollisionLayer()))
        return;
    ...
    if (!b1 && !b2 && dGeomGetClass(o1) == dTriMeshClass &&
        dGeomGetClass(o2) == dTriMeshClass)
        return;

    // Don't collide bodies which are joined together (excluding a contact
    // joint) Might be useful for rag dolls. Or might not.
    ...
    s32 numc = dCollide(o1, o2, MAX_CONTACTS_PER_COLLISION, &contact[0].geom,
                        sizeof(dContact));
    ...
    CollisionMaterialInteraction *interaction =
        GetCollisionMaterialInteraction(material1, material2);
    ...
    for (s32 i = 0; i < numc; i++)
    {
        ...
        // (i.e. a bit of code that may alter the interaction settings depending
        // on the collision)
        if (interaction->preparator)
        {
            ...
            Collision collision;
            collision.pos =
                core::vector3df(geom.pos[0], geom.pos[1], geom.pos[2]);
            collision.normal =
                core::vector3df(geom.normal[0], geom.normal[1], geom.normal[2]);
            ...
            // Set the ODE surface using this temporary interaction modified
            // with Prepare.
            SetSurfaceFromInteraction(surface, interactionTemp);
        }
        else
        {
            // No "preparation" done, so just set the ODE surface from the
        ...
        dJointID c =
            dJointCreateContact(world, perStepContactJointGroup, contact + i);
        dJointAttach(c, b1, b2);
    }
}

void Physics::SetSurfaceFromInteraction(
    dSurfaceParameters &surface,
    const CollisionMaterialInteraction &interaction)
{
