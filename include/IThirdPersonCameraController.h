...
public:
    ...
    // The object in question must then not be deleted all the while this camera
    // controller exists.
    virtual void Follow(ITransformable *toFollow) = 0;

    // Special version of Follow that makes following a character easier.
    // Sets the given collider, and automatically adds the character's body to
    // the collider to be excluded from collisions.
    virtual void Follow(ICharacter *toFollow, ICameraCollider *collider) = 0;

    // Yaw, Pitch, Zoom
    // Set an axis to -1 to disable.
    virtual void SetInputAxes(s32 idAxisYaw, s32 idAxisPitch,
                              s32 idAxisZoom = -1) = 0;
    ...
    // By default these are both set to the internal distance, so zoom is
    // disabled.
    ...
    // Add some functionality for dealing with camera collisions with the
    // environment.
    virtual void SetCollider(ICameraCollider *collider) = 0;

    // Creates a simple built in camera collider. This collider sets the actual
    // camera position to the first collision point between the camera target
    // and the desired camera position. So the camera will never go behind or
    // through an object. Useful for third person cameras.
