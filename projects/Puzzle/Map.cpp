...
{
    if (map.count(coord.X) && map[coord.X].count(coord.Y) &&
        ...
        for (const auto &j_map : i_map.second)
        {
            for (const auto &k_map : j_map.second)
            {
                ...
                if (k_map.second.event)
                    eventCoords.push_back(
                        core::vector3di(i_map.first, j_map.first, k_map.first));
            }
        }
    }
...
bool Map::StartObjectMove(core::vector3di source, core::vector3di dest,
                          bool forceMove)
{
    ...
    if (sourceLoc.containsObject && (sourceLoc.object.movable || forceMove) &&
        !sourceLoc.traversing)
    {
        ...
        if (!destLoc.containsObject)
        {
            ...
            destLoc.traversing =
                true; // Must call FinishObjectMove to set this false.
...
bool Map::ObjectMoveWillComplete(core::vector3di source, core::vector3di dest,
                                 bool forceMove)
{
    if (!LocationExists(source))
        return false;
...
void Map::SetObject(core::vector3di coord, ITransformable *object, bool movable,
                    E_OBJECT_TYPE type)
{
    MapLocation &loc = GetLocation(coord);

    // if setting a new object, then the location must NOT already contain an
    // object
    ASSERT((!object) || (!loc.containsObject));

    loc.containsObject = (bool)object; // coud be NULL to clear object
    loc.traversing =
        false; // object is assumed to have been immediately positioned

    MapObject mapObject;
    loc.object = mapObject; // Set MapObject to defaults
                            // Note: do not set MapLocation to defaults as doing
                            // so would also clear event.
...
void Map::SetObjectPreviousCoord(core::vector3di coord,
