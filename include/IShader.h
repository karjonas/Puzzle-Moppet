...
// my shader system. Should really have separate HighLevelShader and
// LowLevelShader derived from a base IShader class. (and two different
// CreateShader methods) And different LowLevelShadersAvailable and
// HighLevelShadersAvailable methods. (or better still, have some kind of even
// more general purpose shader version fallback system)
...
public:
    ...
    void PushSingleRegister(const c8 *name1, const c8 *name2, const c8 *name3,
                            const c8 *name4)
    {
    ...
    // four registers used for a matrix. (4 components per register, gives 4*4 =
    // 16)
    ...
    // (remember, each single register is four values. so to store 8 floats
    // would require 2 registers) NOT IMPLEMENTED YET AS NOT NEEDED YET.
    /*
...
// Perhaps ought to have a predefined selection of constants that can be
// "watched". e.g. world matrix etc So a callback is not needed for common
// things. IShader->EnableAutoVertexConstant(EAVC_WORLD_MATRIX);
...
public:
    ...
    virtual void SetVertexRegisterMap(
        const LowLevelShaderRegisterMap &registerMap) = 0;
    virtual void SetPixelRegisterMap(
        const LowLevelShaderRegisterMap &registerMap) = 0;

    // Constants set with the Set..Constant methods are stored permanantly and
    // then set by IShader before each render. If you want a constant to be
    // updated before each render, use a callback and the SetCallback method.
    ...
    void SetVertexConstant(const c8 *name, f64 value)
    {
        SetVertexConstant(name, (f32)value);
    }
    void SetPixelConstant(const c8 *name, f64 value)
    {
        SetPixelConstant(name, (f32)value);
    }

    // Extra matrix methods
    void SetVertexConstant(const c8 *name, core::matrix4 &mat)
    {
        SetVertexConstants(name, mat.pointer(), 16);
    }
    void SetPixelConstant(const c8 *name, core::matrix4 &mat)
    {
        SetPixelConstants(name, mat.pointer(), 16);
    }
    ...
    // MaterialTypeParam2 is overwritten in the material to store an ID to this
    // Shader.
    ...
    virtual void ClearFromIrrMaterial(
        video::SMaterial &material, video::E_MATERIAL_TYPE newMaterialType) = 0;
};
