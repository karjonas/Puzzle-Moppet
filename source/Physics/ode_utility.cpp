...
ODEMeshData *ODECreateMeshData(scene::IMesh *mesh,
                               core::vector3df scale = core::vector3df(1, 1, 1))
{
    ...
    for (u32 i = 0; i < mesh->getMeshBufferCount(); i++)
    {
        ...
        case video::EVT_STANDARD:
        {
            ...
            for (u32 j = 0; j < vertexCount; j++)
            {
                vertices[vertexCountTotal + j][0] =
                    iVertices[j].Pos.X * scale.X;
                vertices[vertexCountTotal + j][1] =
                    iVertices[j].Pos.Y * scale.Y;
                vertices[vertexCountTotal + j][2] =
                    iVertices[j].Pos.Z * scale.Z;
        ...
        for (u32 j = 0; j < vertexCount; j++)
        {
            boundingBox.addInternalPoint(vertices[vertexCountTotal + j][0],
                                         vertices[vertexCountTotal + j][1],
                                         vertices[vertexCountTotal + j][2]);
        }
...
#if defined(dSINGLE)
    dGeomTriMeshDataBuildSingle(triMeshData, (const dReal *)vertices,
                                4 * sizeof(dReal), vertexCountTotal, indices,
                                indexCountTotal, 3 * sizeof(int));
#elif defined(dDOUBLE)
    dGeomTriMeshDataBuildDouble(triMeshData, (const dReal *)vertices,
                                4 * sizeof(dReal), vertexCountTotal, indices,
                                indexCountTotal, 3 * sizeof(int));
#endif
