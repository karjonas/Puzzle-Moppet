...
public:
    IPausable() { pauseCounter = 0; }

    virtual ~IPausable() {}

    // If you call Pause, you must also call Resume at a later date, since it is
    // reference counted. e.g. if Pause is called twice, in two different
    // places, the object will not be un-paused until Resume has been called
    // twice.
    void Pause()
    {
        if (pauseCounter == 0)
