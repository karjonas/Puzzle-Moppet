...
void InputProfile::NotifyAxisChanged(s32 id, f32 delta,
                                     const Set<IWantInput *> &subscribers)
{
    if (id != -1)
    {
...
void InputProfile::NotifyButtonDown(s32 id,
                                    const Set<IWantInput *> &subscribers)
{
    if (id != -1)
    {
...
void InputProfile::OnEvent(const Event &event)
{
    ...
    if (virtualAxis != -1 && IsAxisEnabled(virtualAxis))
    {
        ...
        newEvent["delta"] =
            event["delta"].To<f32>() * GetAxisInversionAsFloat(event["axis"]);
...
void InputProfile::ProcessAndNotify(const Set<IWantInput *> &subscribers)
{
    ...
    // We don't use button input events, instead generate our own using button
    // states. otherwise would have to consider missed events when paused.
    ...
    for (std::map<s32, Set<s32>>::const_iterator i =
             reverseButtonBindings.begin();
         i != reverseButtonBindings.end(); i++)
    {
        ...
        // Find if any physical buttons bound to this virtual button is
        // currently down, and also if any were previously down.
        ...
        if (forcedVirtualButtons.count(id))
        {
            ...
            if (forcedVirtualButtons[id] == anyDownNow)
            {
                ...
                // We still continue below, as the button delta event was
                // generated earlier by the call to ForceButtonState.
            }
        ...
        // *any* can be down now, where all were not down previously to generate
        // down event.
        ...
        // *all* must be up, where at least one was down previously, to generate
        // up event.
    ...
    // Here we may generate some ProfileAxisMoved events as a result of button
    // presses.
    for (std::map<s32, Set<s32>>::const_iterator i =
             reverseAxisAsButtonBindings.begin();
         i != reverseAxisAsButtonBindings.end(); i++)
    {
        ...
        // Final result for the axis delta is the sum of all deltas from new
        // real button events.
        f32 axisDeltaTotal = 0.f;

        // Find if any physical buttons bound to this virtual *axis* have been
        // pressed down.
        for (u32 j = 0; j < realButtons.size(); j++)
        {
            ...
            if (newRealButtonStates[realButtons[j]] &&
                !lastRealButtonStates[realButtons[j]])
    ...
    // We do them here rather than inline above because the events could well
    // delete this input profile and we don't want to be accessing member
    // attributes at the time. For that reason we also need to operate on a copy
    // of the vector.

    // Better solution may be to use the new QueueEvent rather than PostEvent
    // below...
    ...
    // but now we do it before, since the button downs may delete this
    // InputProfile. We also queue the event instead of instantly posting it for
    // a similar reason.
    ...
    for (const Event &event : profileEventCacheCOPY)
    {
