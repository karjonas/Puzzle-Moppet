...
core::vector3df quaternion_to_euler(const core::quaternion &quat)
{
    ...
    // The Irrlicht quaternion.toEuler method doesn't seem to account for gimbal
    // lock??
    ...
    if (fabs(sp) > 0.9999f)
    {
        ...
        euler.Y = atan2(-quat.X * quat.Z + quat.W * quat.Y,
    ...
    else
    {
        ...
        euler.Z = atan2(quat.X * quat.Y + quat.W * quat.Z,
...
{
    // I think the reason I interpolate position and rotation separately (rather
    // than using matrix.interpolate) is for slerp.

    dest.setTranslation(
        core::lerp(a.getTranslation(), b.getTranslation(), alpha));
    dest.setRotationDegrees(interpolate_rotation(a.getRotationDegrees(),
                                                 b.getRotationDegrees(),
                                                 alpha));
}
...
{
    return quaternion_to_euler(core::quaternion(a * core::DEGTORAD) *
                               core::quaternion(b * core::DEGTORAD));
}
                               ...
                               const core::vector3df &b)
{
    ...
    // fix: otherwise the dot product never hits exactly 1.0 (and thus acos(dp)
    // never reaches 0.0). probably due to rounding errors
    if (q1 == q2)
        return 0.0;
...
f32 extract_y_rotation(const core::vector3df &rot)
{
    ...
    return vec2d.getAngle() +
           90.0; // Irrlicht has 0 to the right (1,0) rather than up (0,1).
}
...
void SetAbsoluteRotation(scene::ISceneNode *node, core::vector3df absoluteRot)
{
    ...
    core::quaternion qOldRotAbs(
node->getAbsoluteTransformation().getRotationDegrees() * core::DEGTORAD );
