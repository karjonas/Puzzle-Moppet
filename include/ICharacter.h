...
public:
    ...
    // only the sign is used. Positive for forwards movement, negative for
    // backwards, zero for no movement
    ...
    // A value used for smooth rotation of the character when turning to a new
    // angle. Default 10.0 Set to 0 to disable (immediately looks to new angle).
    virtual void SetTurnSmoothing(f32 smooth) = 0;

    // Get a normalised vector of the attempted motion of the character in the
    // last update. This is a result of control (AI or key input), the move
    // vector, and medial or lateral motions. This is the *attempted* move, not
    // the actual move achieved (the move may be obstructed by a collision).
    // Does not consider "accidental" movement due to external things like
    // physics.
