...
public:
    ...
    // Add a user implemented transformable. (enables things like animators and
    // scene graph).
    ...
    // Removes all *except* the camera, which is a special case ITransformable
    // with only ever one instance.
    ...
    // removeOnDestruct - if true, will remove the Irrlicht scene node when it
    // is itself removed.
    virtual INodeHandler *AddIrrNodeHandler(scene::ISceneNode *irrNode,
                                            bool removeOnDestruct = true) = 0;

    // A dummy node that does nothing except provide a transform.
    virtual ITransformable *AddTransform() = 0;

    // If height is not given then the character body geometry will be
    // determined from the mesh bounding box.
    virtual ICharacter *AddCharacter(IMesh *mesh, s32 idAnimIdle,
                                     s32 idAnimWalk, f32 height = 0.0) = 0;
    ...
    virtual IProxyTransformable *AddProxyTransformable(
        ITransformable *other) = 0;
    ...
    // button list, and the moveRelativeTo Entity (e.g. a third person camera
    // entity). Uses the Entity's angle. Without a moveRelativeTo entity given,
    // the character moves only along the X,Z axes.
    virtual ICharacterController *CreateUserCharacterController(
        int idButtonWalkForwards, int idButtonWalkBackwards,
        int idButtonWalkLeft, int idButtonWalkRight,
        ITransformable *moveRelativeTo = nullptr) = 0;

    virtual IThirdPersonCameraController *CreateThirdPersonCameraController(
        f32 distance = 8.0, f32 heightOffset = 0.0, f32 minPitch = -60.0,
        f32 maxPitch = 60.0, f32 turnSpeed = 120.0) = 0;
    ...
    // Some world effects
    virtual void SetSkyBox(const core::stringc &top,
                           const core::stringc &bottom,
                           const core::stringc &left,
                           const core::stringc &right,
                           const core::stringc &front,
                           const core::stringc &back) = 0;

    // Assuming a sky box has already been set, this allows you to set a shader
    // for it. Sets for all materials.
