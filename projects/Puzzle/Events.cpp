...
DefaultEvent::DefaultEvent(Level *level)
    : IMapEventOwner(level)
{
...
void DefaultEvent::MoveStack(const core::vector3di &belowStack,
                             const core::vector3di &moveDir)
{
    if (moveDir.Y == 0) // only move stack when moving in horizontal plane. (not
                        // when falling)
    {
        ...
        while (true)
        {
            ...
            // Hit a balloon! These are considered weightless, and may support
            // other blocks on top of themselves, so we don't move them.
            if (map->GetObjectType(stackPos) == EOT_BALLOON)
                break;

            // Ok, it's a block we can move.
            // Try to move it.
            // If the move fails, (e.g. the destination contains an object), we
            // don't move any more of the stack above it.
    ...
    if (map->GetObject(coord) && map->GetIsObjectSliding(coord))
    {
        ...
        case EOT_SLIDING_BLOCK:
            ...
            if (map->ObjectHasPreviousCoord(coord))
            {
                ...
                if (!(eventType == EET_FAN_EVENT ||
                      eventType == EET_LIFT_EVENT))
                {
                    core::vector3di slideDir =
                        coord - map->GetObjectPreviousCoord(coord);
        ...
        case EOT_BALLOON:
            ...
            if (map->ObjectHasPreviousCoord(coord))
            {
                ...
                if (!(eventType == EET_FAN_EVENT)) // || eventType ==
                                                   // EET_LIFT_EVENT) )
                {
                    core::vector3di slideDir =
                        coord - map->GetObjectPreviousCoord(coord);
...
void DefaultEvent::OnMapMoveEvent(core::vector3di coord)
{
    if (map->GetObject(coord))
    {
        ...
        // So we want to find all blocks stacked above it on the previous
        // location, and move them too. (all blocks stacked until we reach an
        // immovable block or a weightless balloon)
        case EOT_SLIDING_BLOCK:
        case EOT_BALLOON:
        {
            core::vector3di prevCoord = map->GetObjectPreviousCoord(coord);

            MoveStack(prevCoord, coord - prevCoord);
        }
        break;
        default:
            break;
...
// This emitter causes an entire seconds worth of particles to be emitted at
// once, after which the emitter stops emitting.
...
public:
    ...
    scene::E_PARTICLE_EMITTER_TYPE getType() const override
    {
        return scene::EPET_POINT;
    }
};
...
void DefaultEvent::OnMapLeaveEvent(core::vector3di coord,
                                   core::vector3di newCoord)
{
    // There may not be any object on this coord since the object has just left
    // it! Object *may* be on newCoord, although I'm not 100% certain it will
    // be, so I must GetObject check before using.
    ...
    if (map->GetObject(below) &&
        map->GetObjectType(below) == EOT_GROUND_BLOCK_FALL)
    {
        // And check that the object that just moved was not a weightless
        // balloon... (but if the object is not present in the new location,
        // then continue regardless)
        if (!map->GetObject(newCoord) ||
            map->GetObjectType(newCoord) != EOT_BALLOON)
        {
            // Just remove...
            ...
            scene::ISceneManager *smgr =
                GetEngine()->GetIrrlichtDevice()->getSceneManager();
            video::IVideoDriver *driver =
                GetEngine()->GetIrrlichtDevice()->getVideoDriver();
            ...
            scene::IParticleEmitter *emitter = ps->createBoxEmitter(
                ...
                core::vector3df(0, -0.00005, 0), 20, 30,
                ...
                core::dimension2df(minParticleSize, minParticleSize), // minimum
                                                                      // size
                core::dimension2df(maxParticleSize, maxParticleSize)  // maximum
                                                                      // size
            );

            // Emit entire one second's worth of particles all at once.
            // Then no more...!
            scene::IParticleEmitter *singleEmissionEmitter =
                new SingleEmissionEmitter(emitter);
            emitter->drop();
            ...
            scene::ISceneNodeAnimator *deletionAnim =
...
bool DefaultEvent::RequestActionPermission(core::vector3di coord,
                                           E_ACTION_TYPE action)
{
    ...
    if (action == EAT_FALL && map->GetObject(coord) &&
    ...
    if (action == EAT_MOVE_BY_PUSH)
    {
        ...
        if (map->GetObject(above)
            ...
            && (!map->GetObject(coord) ||
                (map->GetObjectType(coord) != EOT_SLIDING_BLOCK &&
                 map->GetObjectType(coord) != EOT_BALLOON)))
        {
            return false;
        }
...
void FanEvent::OnMapEvent(core::vector3di coord)
{
    ...
    // How to determine if top event: This same event will not be set for the
    // square above.
    ...
    if (map->GetObject(coord) && map->GetObjectType(coord) == EOT_PLAYER_CENTRE)
    {
                ...
                dynamic_cast<IProxyTransformable *>(map->GetObject(coord)))
        {
            ...
            {
                // Hacked in.
                // So it looks better when the player gets stuck with a block
                // below (if he's standing upright, why can't he just walk off
                // the block!?)
    ...
    // this flag just used for the player, to detect when he has stopped moving
    // upwards in a fan updraft, so can send restart level event and make him
    // bob. Finished moving upwards either when there are no more fan events
    // left, or if the upwards move attempt fails (i.e. due to obstruction by
    // another object that is above the player in the fan vortex)
    ...
    if (finishedMovingUpwards)
    {
            ...
            map->GetObjectType(coord) == EOT_PLAYER_CENTRE)
        {
                    ...
                    dynamic_cast<IProxyTransformable *>(map->GetObject(coord)))
            {
                ...
                {
                    IBobAnimator *anim =
                        GetEngine()->GetWorld()->CreateBobAnimator(0.1, 1.0);
...
void FanEvent::OnMapLeaveEvent(core::vector3di coord, core::vector3di newCoord)
{
    ...
    // This move will in turn generate a OnMapLeaveEvent for the location below
    // this one. So all blocks will move up correctly.
...
bool FanEvent::RequestActionPermission(core::vector3di coord,
                                       E_ACTION_TYPE action)
{
    switch (action)
    {
...
void LiftEvent::MoveInDirection(core::vector3di liftCoord,
                                E_MOVE_DIRECTION moveDir)
{
    if (moveDir == EMD_DOWN)
    {
        // Just move the lift down one square. Gravity will move any objects
        // resting on it later.
        level->StartMapObjectMove(liftCoord,
                                  liftCoord + core::vector3di(0, -1, 0), true);
    }
    else
    {
        // Moving up is more complicated.

        // iterate upwards from the square above the lift, until either an
        // immovable object, or an empty (no object) square is found.
        ...
        while (1)
        {
            ...
            if (!object)
            {
                ...
                while (1)
                {
                    ...
                    // forceMove is set to enable moving of the usually
                    // immovable Lift
                    level->StartMapObjectMove(coord,
                                              coord + core::vector3di(0, 1, 0),
                                              true);
...
void LiftEvent::OnMapEvent(core::vector3di coord)
{
    ...
    - is the lift on the square below at an end location (either bottom event
    square or one from top, see diagram)

    So attempt to move.

    Determine direction to move, up or down:
    - if the square below the lift is NOT a lift event (not event == this), then
    should move upwards
    - if the square two above the lift is NOT a lift event, then should move
    downwards
    - [if both upwards and downwards, then do nothing...]

    Actual move:
    - if moving down, then just move the lift down one square. Gravity will do
    the rest.
    - if moving up, do as follows.

    Moving up:
    - iterate upwards from the square above the lift, until either an immovable
    object, or an empty (no object) square is found.
    - if an immovable object is found, cannot move, so do nothing.
    - if an empty square is found, then iterate downwards again, moving each
    object upwards by one, including the lift when it is reached. (end iteration
    at the lift)

    TEST THIS BEFORE IMPLEMENTING OTHER CASES.
    - test with lift at top and bottom of path, with zero, one, two, and more
    objects stacked on it(???)
    - correct operation should be for the lift to move one square (up or down as
    the case may be) and then stop. All objects resting on the lift to move as
    well.
    ...
    */
    ...
    // removed assertion, as sometimes this *is* getting called on a square with
    // no event. not sure how...
    if (!object)
        return;
    ...
    )
    {
        ...
        bool moveUp =
            map->GetEvent(liftCoord + core::vector3di(0, -1, 0)) != this;

        // Square two above lift is NOT a LiftEvent, at top so move downwards.
        bool moveDown =
            map->GetEvent(liftCoord + core::vector3di(0, 2, 0)) != this;
        ...
        {
            // need also to check that the lift has not only just reached its
            // end location if going up, check that object last position was NOT
            // one below if going down, check that object last position was NOT
            // one block above if object has no previous coordinate, i.e. was
            // positioned initially on this location, then will move.

            if (moveUp && (!map->ObjectHasPreviousCoord(coord) ||
                           map->GetObjectPreviousCoord(coord) !=
                               coord + core::vector3di(0, 1, 0)))
            {
                MoveInDirection(liftCoord, EMD_UP);
                level->LiftMoveInc();
            }
            else if (moveDown && (!map->ObjectHasPreviousCoord(coord) ||
                                  map->GetObjectPreviousCoord(coord) !=
                                      coord + core::vector3di(0, -1, 0)))
            {
    ...
    We can assume as the lift is not at an end location that it has moved at
    least once on a journey from one end to the other. So we must determine the
    direction to continue moving in...

    - determine move direction (using lift GetPreviousCoordinate. May need to
    implement the Map method for this).
    - move in that direction (moving as in (A))

    CheckForEndlocation and MoveInDirection could be implemented as separate
    methods as the functionality is common to both (A) and (B).

    TEST.
    - test similar to (A) except now the lift should continue to its
    destination.
    ...
    */
    ...
    {
        // We can assume since the lift is not at an end location it has moved
        // at least once on a journey from one end to another
    ...
    - is the lift on the square below NOT at an end location	] this differs
    from (A)

    Determine if the lift can move in EITHER direction:
    - for going down, check if the square below the lift is empty. If so, it can
    move ok.
    - for going up, do iteration upwards as in (A). If an empty square is
    reached BEFORE an immovable object, can move ok.

    If the lift can only move in one direction, then this is the direction it
    should move in!
    - MoveInDirection as in both (A) and (B).

    Note that in theory (B) and (C) could both be called here. This is not a
    problem as moves will just fail if the objects have already had their moves
    started.
    ...
    */
    ...
    if (map->GetObject(coord) // need to first check for object again, in case
                              // it was moved in (B)
    ...
    )
    {
        ...
        // Can move up?
        // Iterate upwards, if an empty square is reached before an immovable
        // object, then can move upwards.
        ...
        bool moveDown =
            map->GetObject(liftCoord + core::vector3di(0, -1, 0)) == nullptr;
...
bool LiftEvent::RequestActionPermission(core::vector3di coord,
                                        E_ACTION_TYPE action)
{
    switch (action)
    {
    ...
    case EAT_LOCOMOTE:
        ...
        // FIX: Need this otherwise player can get trapped when standing on a
        // block that is on a lift. But was there a good reason for the old
        // behaviour???!
        return true;
