...
public:
    ...
    // This virtual time is the same as the virtual time of the parent
    // IUpdatable that owns this updater.
    virtual f32 GetVirtualTime() const = 0;

    // Return the last delta time...
    // If this is called from within a child updatable's Update method, this
    // will be the *current* delta time.
