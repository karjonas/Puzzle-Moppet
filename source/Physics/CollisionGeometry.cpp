...
CollisionGeometry::CollisionGeometry(dSpaceID space)
{
    ...
    // Geom is set by a call to SetODEGeom in the constructor of derived
    // classes.
    geom = nullptr;
}
...
void CollisionGeometry::SetODEGeom(dGeomID geom)
{
    ...
    // Pointer must be to ICollisionGeometry as that is what we cast back to
    // when getting the data. (If the types don't match exactly we get insidious
    // crashes.)
...
core::aabbox3df CollisionGeometry::GetAABB()
{
    ...
    return core::aabbox3df(aabb[0], aabb[2], aabb[4], aabb[1], aabb[3],
                           aabb[5]);
}
