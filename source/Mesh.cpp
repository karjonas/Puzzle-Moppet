...
core::aabbox3df Mesh::GetBoundingBox()
{
    ...
    // (the AABB got from the meshNode is the BB of the completely untransformed
    // animated mesh. so no position, rot or scale will have been applied to
    // it).
...
void Mesh::SetShader(u32 material, IShader *shader, u16 pass)
{
    ...
    else if (passes[pass].shaders[material])
    {
        ...
        passes[pass].shaders[material]->ClearFromIrrMaterial(
            passes[pass].materials[material], video::EMT_SOLID);
    }
...
void Mesh::SetCurrentPass(u16 pass)
{
    ...
    else
    {
        ...
        // (should not use meshNode->setVisible() as that effects child nodes
        // too)
        for (u32 i = 0; i < meshNode->getMaterialCount(); i++)
        {
    ...
    if (pass == 0)
    {
        ...
        if (currentAnimation)
        {
            ...
            if (onFirstFrame)
            {
                // We need some special behaviour for the first frame after
            setting the frame loop
            ...
            */
            ...
            if (!currentAnimation->looped &&
                meshNode->getFrameNr() >= currentAnimation->end - 1)
            {
                HaltAnimation();
            }
        }
    ...
    if (animatedMesh)
    {
        ...
        if (mesh)
        {
            ...
            if (passes.size() > 1)
            {
                ...
                ASSERT(passes[passes.size() - 1].materials.size() ==
                       passes[0].materials.size());
            }
        }
    }
}
...
void Mesh::DefineAnimation(s32 id, s32 start, s32 end, f32 fps, bool looped,
                           bool interruptible, f32 transitionTime)
{
    Animation anim = {start, end, fps, looped, interruptible, transitionTime};
