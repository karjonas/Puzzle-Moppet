...
public:
                          ...
                          core::vector3df &resultPos) override
    {
        ...
        if (physics->RayCastExcluding(cameraRay, excludedGeometries,
                                      &collisionResult, collisionLayer))
        {
...
ThirdPersonCameraController::ThirdPersonCameraController(
    IWorld *world, scene::ISceneManager *smgr, f32 distance, f32 heightOffset,
...
void ThirdPersonCameraController::Follow(ICharacter *toFollow,
                                         ICameraCollider *collider)
{
    ...
    if (following && collider)
        collider->ExcludeGeometry(
            toFollow->GetBody()->GetCollisionGeometries());
}

void ThirdPersonCameraController::SetInputAxes(s32 idAxisYaw, s32 idAxisPitch,
                                               s32 idAxisZoom)
{
...
IThirdPersonCameraCollider *
    ThirdPersonCameraController::CreateThirdPersonCameraCollider()
{
...
void ThirdPersonCameraController::Update(ICamera *camera, f32 dt)
{
    ...
    else
    {
        ...
        currentAngle.X = maths::interpolate_angle(currentAngle.X, nextAngle.X,
                                                  dt * turnSmooth);
        currentAngle.Y = maths::interpolate_angle(currentAngle.Y, nextAngle.Y,
                                                  dt * turnSmooth);
    }

    core::quaternion quat(core::vector3df(currentAngle.X, currentAngle.Y, 0.0) *
                          core::DEGTORAD);
    ...
    if (collider)
    {
        ...
        bool collisionOccurred =
            collider->ProcessCollision(core::line3df(targetPos, cameraPos),
                                       cameraPos);
        ...
        camera->SetRotation(
            core::vector3df(currentAngle.X, currentAngle.Y, 0.0));

        // Set the distance to target point.
        // Will then keep looking exactly at this point during interpolation.
        // If a collision has occurred, we need to recalculate the distance
        // using the changed cameraPos.
        camera->SetTargetDistance(collisionOccurred
                                      ? (cameraPos - targetPos).getLength()
                                      : currentDistance);
    }
