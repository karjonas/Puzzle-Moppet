...
// this class is inspired by the Variant class from the zlib licensed
// CRTMPServer, http://www.rtmpd.com
class Variant
{
public:
    enum E_VARIANT_TYPE
    {
        ...
        EVT_F64,
        EVT_STRING,
        EVT_VECTOR // vector is not implemented for a lot of conversions that
                   // don't make sense.
    };
...
private:
    ...
    // (and I don't forget to copy other things if I expand Variant with more
    // class types)
    struct
    {
        // Ideally, string might be dynamically allocated, with the pointer kept
        // in the union above. But this is simpler, and I'm not bothered about
        // performance.
        core::stringc _str;
...
public:
    ...
    template <class Type>
    Variant(const std::vector<Type> &value)
    {
    ...
    // Note: it's safer not have a const char * cast operator in case the
    // Variant goes out of scope. (thus making any pointers to the temporary
    // character array invalid)
    ...
    template <class Type>
    Variant &operator=(const std::vector<Type> &value)
    {
        variantType = EVT_VECTOR;
    ...
    template <class Type>
    bool operator==(const std::vector<Type> &value) const
    {
