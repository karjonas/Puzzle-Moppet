...
core::vector3df Camera::GetInterpolatedPosition(f32 alpha)
{
    ...
    f32 interpolatedTargetDistance =
        cachedTargetDistance + (targetDistance - cachedTargetDistance) * alpha;
...
void Camera::ReceiveRenderRotation(core::vector3df rot)
{
    ...
    // (done in ICameraSceneNode::setRotation when target and rotation are
    // bound)
