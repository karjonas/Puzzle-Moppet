...
// (since DefaultEvent inherits IPausable, and thus must be dropped and not
// deleted)
{
protected:
    Level *level;
...
public:
    ...
    // Hacked in.
    // Called whenever an event location is left. (an object moves off this
    // location) coord - the event location that previously had an object in it.
    // newCoord - the location that now should contain the object.
    //		I have not actually verified that the object will always be at
    // newCoord...
    virtual void OnMapLeaveEvent(core::vector3di coord,
                                 core::vector3di newCoord) = 0;

    // When an object is on this event square, it must request permission of
    // this event owner to perform actions. THIS SHOULD ONLY BE CALLED ON
    // DefaultEvent. Default event in turn calls this for map location specific
    // events.
    virtual bool RequestActionPermission(core::vector3di coord,
                                         E_ACTION_TYPE action) = 0;
...
// NOTE: There is only ever one instance of an event; state is kept in map and
// other places.
...
class DefaultEvent : public IMapEventOwner, public IPausable
{
    ...
    void MoveStack(const core::vector3di &belowStack,
...
public:
    ...
    void OnMapLeaveEvent(core::vector3di coord,
                         core::vector3di newCoord) override;

    bool RequestActionPermission(core::vector3di coord,
                                 E_ACTION_TYPE action) override;

    E_EVENT_TYPE GetType() override { return EET_DEFAULT_EVENT; }

    core::stringc GetIconMeshName() override
    {
...
public:
    ...
    core::stringc GetIconMeshName() override
    {
        return core::stringc("event_fan_draft.b3d");
    }
};
...
public:
    ...
    core::stringc GetIconMeshName() override
    {
        return core::stringc("event_lift.b3d");
    }
};

// Dummy event, does nothing. Used to indicate a position that the player can
// start the level from.
class PlayerStartEvent : public IMapEventOwner
{
public:
    PlayerStartEvent(Level *level)
        : IMapEventOwner(level)
    {
    }
    ...
    core::stringc GetIconMeshName() override
    {
        return core::stringc("event_player_start.b3d");
    }
};
