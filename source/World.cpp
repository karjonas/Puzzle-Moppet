...
World::World()
{
    ...
    camera = new Camera(
        engine->GetIrrlichtDevice()->getSceneManager()->getActiveCamera());
    AddTransformable(camera);
    // Do NOT drop camera yet, as we need to keep it even if it is removed from
    // transformables list. (it is only finally dropped in destructor)
...
World::~World()
{
    ...
    // Must set camera to NULL so it is not added back in by
    // RemoveAllTransformables (otherwise RemoveAllTransformables adds camera
    // back to transformables after removing all).
    camera = nullptr;
...
void World::AddTransformable(ITransformable *transformable)
{
    ...
    // i.e. give the transformable a pointer to the world. So it can remove
    // children.
...
{
    IMesh *mesh =
        new Mesh(engine->GetIrrlichtDevice()->getSceneManager(), meshName);
    AddTransformable(mesh);
    mesh->drop();
    return mesh;
}

INodeHandler *World::AddIrrNodeHandler(scene::ISceneNode *irrNode,
...
ICharacter *World::AddCharacter(IMesh *mesh, s32 idAnimIdle, s32 idAnimWalk,
                                f32 height)
{
    ICharacter *character =
        new Character(this, physics, mesh, idAnimIdle, idAnimWalk, height);
...
ICharacterController *World::CreateUserCharacterController(
    int idButtonWalkForwards, int idButtonWalkBackwards, int idButtonWalkLeft,
    int idButtonWalkRight, ITransformable *moveRelativeTo)
{
    return new UserCharacterController(this, idButtonWalkForwards,
                                       idButtonWalkBackwards, idButtonWalkLeft,
                                       idButtonWalkRight, moveRelativeTo);
}

IThirdPersonCameraController *World::CreateThirdPersonCameraController(
    f32 distance, f32 heightOffset, f32 minPitch, f32 maxPitch, f32 turnSpeed)
{
    return new ThirdPersonCameraController(
        this, engine->GetIrrlichtDevice()->getSceneManager(), distance,
        heightOffset, minPitch, maxPitch, turnSpeed);
}
...
void World::SetSkyBox(const core::stringc &top, const core::stringc &bottom,
                      const core::stringc &left, const core::stringc &right,
                      const core::stringc &front, const core::stringc &back)
{
    ...
    skyBoxNode = smgr->addSkyBoxSceneNode(driver->getTexture(top.c_str()),
...
void World::SetSkyBoxShader(IShader *shader)
{
    if (skyBoxNode)
    {
        ...
        else
        {
            ...
            {
                for (u32 i = 0; i < skyBoxNode->getMaterialCount(); i++)
                    skyBoxShader->ClearFromIrrMaterial(skyBoxNode->getMaterial(
                                                           i),
                                                       video::EMT_SOLID);
            }
        }
...
void World::OnPause()
{
    ...
    // This will cause a millisecond pause when resuming, but that is
    // acceptable.
...
void World::Update(f32 dt)
{
    ...
    // This is because a child in the scene node may rely on its parent's
    // interpolated transform, yet parent does not have to be a graphic. So all
    // nodes must be able to be interpolated if necessary.
    ...
    for (auto &elem : transformables)
    {
        ...
        // (and the final position may change at various points during this
        // Update)
    ...
    // Update other updatables after all ITransformables but before camera in
    // case any Updatables change the positions of any characters etc.. (camera
    // will want to watch these new positions)
    ...
    if (ITransformable *listener = camera)
    {
        ...
        soundSystem->SetListenerOrientation(
            // Look vector
            listener->GetAbsoluteRotation().rotationToDirection(
